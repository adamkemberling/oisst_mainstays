...
ggplot(aes(time,  sst, group = hw_method)) +
  geom_line(aes(color = status), linewidth = 0.75) +
...
flag_test  %>% 
  ...
  geom_path(aes(color = status), linewidth = 0.25, group = 1) +
  scale_color_manual(values = c( gmri_cols("gmri blue")[[1]], gmri_cols("orange")[[1]],  "gray80")) +
  facet_grid(baseline~hw_type) +
...
## De-trended Timeseries - Anomalies from Reference Period
...
## De-trended Timeseries - Anomalies After De-Trending
