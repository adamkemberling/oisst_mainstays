...
week_summaries <- season_data %>%
  ...
  group_by(`Week`) %>%
  summarise(
    `Observed SST` = mean(area_wtd_sst),
    `Climate Average` = mean(area_wtd_clim),
    `Temperature Anomaly` = mean(area_wtd_anom),
    .groups = "drop") %>%
  mutate(
    temp_f = as_fahrenheit(`Observed SST`),
    clim_f = as_fahrenheit(`Climate Average`),
    anom_f = as_fahrenheit(`Temperature Anomaly`, data_type = "anomalies")) %>% 
  mutate(across(where(is.numeric), round, 2)) %>% 
  mutate(
    temp_both = str_c(`Observed SST`, " / ", temp_f),
    clim_both = str_c(`Climate Average`, " / ", clim_f),
    anom_both = str_c(`Temperature Anomaly`, " / ", anom_f)
  )
...
The temperatures observed this year, and how many heatwaves have occurred can be viewed below:
...
Changing the axis to anomalies rather than temperature looks like this:
...
ranks_map <- ggplot() +
  ...
  scale_fill_viridis_c(limit = c(0.85, 1), 
