...
## Impact of a New Climate Reference Period
...
# tlab_breaks <- seq(-.1, .1, 0.05)
# tlabs <- tlab_breaks
# tlabs[[1]] <- str_c("< ", tlabs[[1]])
# tlabs[[length(tlabs)]] <-  str_c(tlabs[[length(tlabs)]], " <")
# tlabs <- str_c(tlabs, deg_c)


# Use terra for discrete breaks
rates_terra <- terra::rast(rotate(rates_nc))
...
ggplot() +
  tidyterra::geom_spatraster_contour_filled(
    data = rates_terra,
    #breaks = seq(-.1, .2, 0.05), 
    breaks = c(-0.2, -.5, -0.01, -0.005, 0.005, .01, .5, .2),
    maxcell = 10e6,
    alpha = 0.75,
    linewidth = 0.25,
    color = "gray20") +
  ...
  guides(fill = guide_colorsteps(
    ...
    barwidth = unit(12, "cm"), show.limits = T)) +
  labs(x = "", y = "", 
       fill = str_c("1982-2022 Warming Rate (", deg_c, "/year)"))
...
### Locating Highest daily anomaly
