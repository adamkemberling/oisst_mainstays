...
sst_areas <- get_timeseries_paths("gmri_sst_focal_areas", box_location = "cloudstorage")
...
nelme_shape <- get_timeseries_paths("lme", box_location = "cloudstorage")$northeast_us_continental_shelf$shape_path
nelme_shape <- read_sf(nelme_shape)

ss_shape <- get_timeseries_paths("lme", box_location = "cloudstorage")$scotian_shelf$shape_path
ss_shape <- read_sf(ss_shape)

bf_shape <- get_timeseries_paths("gom physio regions", box_location = "cloudstorage")$bay_of_fundy$shape_path
...
world_ts <- read_csv(paste0(oisst_path, "global_timeseries/global_anoms_1991to2020.csv"), 
    guess_max = 1e6,
    col_types = cols()) %>% 
  mutate(year = year(time))
nelme_ts <- oisst_access_timeseries("lme", "northeast us continental shelf", box_location = "cloudstorage")
gom_ts   <- oisst_access_timeseries(region_family = "gmri focus areas", "apershing gulf of maine", box_location = "cloudstorage")
ss_ts    <- oisst_access_timeseries(region_family = "lme", "scotian shelf", box_location = "cloudstorage")
bf_ts    <- oisst_access_timeseries(region_family = "gom physio regions", "bay of fundy", box_location = "cloudstorage")
lis_ts   <- oisst_access_timeseries("gmri focus areas", "long island sound", box_location = "cloudstorage")
...
area_hw <- map(area_hw, function(.x){
  .x %>% mutate(
    ...
    Year = year(time),
    sst_f = gmRi::as_fahrenheit(sst),
    seas_f = as_fahrenheit(seas),
    anom_f = sst_f - seas_f)
})
...
####  1. Yearly Averages Data

# Prep yearly data to export
report_data <- area_hw %>%
  imap(function(ugly_dat, region){

    ugly_dat %>%
      filter(time > as.Date("1982-01-01"),
             time < as.Date("2020-12-31")) %>%
      mutate(mask_region = region,
             above_20 = ifelse(sst > 20, T, F)) %>%
      group_by(mask_region, Year) %>%
      summarise(
        ...
        days_over_20C = sum(above_20, na.rm = T),
        .groups = "drop") %>%
   mutate_if(.tbl = ., .predicate = is.numeric, .funs = round, 4)
...
report_data %>% iwalk(function(out_data, region_name){
  ...
  save_location <- paste0(here("docs/data_products/yearly_data"), "/", clean_name)
  # write_csv(out_data, save_location)
  #print(save_location)

})

# # Plot check the days over 20C
# ggplot(report_data$`Long Island Sound`, aes(Year, days_over_20C)) +
#   geom_col()

#### Prep daily files  ####

# # Pull out areas we want daily data for
daily_exports <- area_hw[c("Long Island Sound", "Gulf of Maine", "Scotian Shelf", "Bay of Fundy")]


# Clean up the data
daily_exports <- imap(daily_exports, function(daily_data, region){
  daily_data %>%
    # filter(time > as.Date("1982-01-01"),
    #        time < as.Date("2021-06-30")) %>%
    transmute(
      mask_region = region,
      date = time,
      temp_c = sst,
      climate_mean_c = seas,
      temp_anom_c = sst_anom,
      hw_thresh_c = mhw_thresh,
      temp_f = sst_f,
      climate_mean_f = seas_f,
      temp_anom_f = anom_f,
      hw_thresh_f = as_fahrenheit(hw_thresh_c),
      is_hw_day = mhw_event,
      above_20c = ifelse(temp_c > 20, TRUE, FALSE)) %>%
    mutate_if(is_numeric, round, 4)
})

# # Plot check
# daily_exports$`Long Island Sound` %>%
#   ggplot(aes(date, temp_c, color = above_20c)) +
#   geom_path(group = 1)

# Save the data to zip up later
daily_exports %>% iwalk(function(out_data, region_name){
  clean_name <- str_replace_all(region_name, " ", "_")
  clean_name <- tolower(clean_name)
  clean_name <- paste0(clean_name,"_daily_oisst.csv")
  save_location <- paste0(here("docs/data_products/daily_data"), "/", clean_name)
  # write_csv(out_data, save_location)
  #print(save_location)
...
lme_rates <- map_dfr(lme_8220, function(lme_data){
  ...
  lme_yearly <- group_by(lme_data, year) %>% summarise(sst = mean(area_wtd_sst, na.rm = T))
...
**Plotting Absolute Changes 1982-2020**


```{r, fig.height=8}

# Get The changes from 1982-2011
lme_absolute_change <- map_dfr(lme_8220, function(lme_data){
  
  # Uses area weighted SST to get yearly averages
  lme_yearly <- lme_data %>% 
    filter(year %in% c(1982, 2020)) %>% 
    group_by(year) %>% 
    summarise(area_wtd_sst = mean(area_wtd_sst, na.rm = T), 
              .groups = "drop")  %>% 
  ...
  mutate(Region = str_replace_all(Region, "_", " "),
         ...
         absolute_change = `2020` - `1982`)

# Resort factor levels
# Pull out the top x and plot
lme_absolute_change <- lme_absolute_change %>% 
  mutate(Region = fct_reorder(Region, `1982`, .desc = T))


# Pull out the top x and plot
ggplot(lme_absolute_change, aes(x = `1982`, xend = `2020`, y = fct_rev(Region))) +
...
data_window <- data.frame(lon = c(-720, 720),
                          lat = c(-90, 90),
                          time = as.Date(c("2021-01-01", "2021-07-31")))
...
plot_months <- month.abb[1:7] # For Animating through each
