...
annual_temps_plot <- ggplot(annual_summary, aes(year, area_wtd_f)) +
  ...
  geom_mark_ellipse(
    ...
    label.fill = "transparent", 
    label.fontsize = 8) +
  geom_mark_ellipse(aes(filter = year == 2021, 
                      label = anom_label),
                  color = "darkred",
                  linetype = 2,
...
gom_anomaly_bars <- annual_summary %>% 
  ggplot(aes(yr_as_dtime, area_wtd_anom_f)) +
    ...
    theme_gmri() +
    theme(legend.position = c(0.85, .125),
...
The Gulf of Maine has a seasonal temperature cycle that experiences its lowest temperatures around March, and its peak summer highs in August. The difference between these two seasons is typically `r diffs_f$clim_diff`$^oF$ or `r diffs_c$clim_diff`$^oC$. In 2021 the temperature range was `r ranges_f$temp_range[1]`-`r ranges_f$temp_range[2]`$^oF$ or `r diffs_f$temp_diff`$^oF$, with temperature anomaly values ranging from `diffs_f$anom_range[1]`-`r diffs_f$anom_range[2]`$^oF$.
...
year_mets <- grid_data %>% 
  ...
  pivot_longer(names_to = "Var", values_to = "Metric", 
               cols = c(`Average Temperature`,`Temperature Above Average`))
