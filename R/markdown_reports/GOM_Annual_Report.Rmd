...
Thanks to some long-running temperature monitoring programs we are able to track monthly sea surface temperatures as far back as 1850. From this view we can see that recent temperatures are the warmest we've ever seen. 2021 setting the new record for the warmest temperature, taking the title away from
...
ggplot() +
 ...
 geom_line(data = oisst_smooth, aes(x, y, color = "OISSTv2"),
  ...
  labs(x = "", y = "Sea Surface Temperature", 
       color = "Temperature Data Record",
       title = "Long-Term Temperature Record for the Gulf of Maine",
       subtitle = "OISSTv2 Data Mirrors ERSSTv5 Timeline with Slight Warm Bias") +
...
## Acceleratated Warming in Recent Years
...
ggplot(data = annual_summary, aes(yr_as_dtime, area_wtd_anom)) +
  ...
  # geom_text(data = data.frame(), 
  #           aes(label = eq_global, x = min(region_timeseries$time), y = Inf),
  #           hjust = -0.25, vjust = 7.5, color = gmri_cols("green")) +
  geom_text(data = data.frame(), 
            aes(label = eq_all, x = min(region_timeseries$time), y = Inf),
            hjust = -0.25, vjust = 8.5, color = gmri_cols("gmri blue")) +
  geom_text(data = data.frame(), 
            aes(label = eq_15, x = min(region_timeseries$time), y = Inf),
            hjust = -0.25, vjust = 10, color = gmri_cols("orange")) +
  ...
  # labels + theme
  labs(title = "Gulf of Maine: Rapid Recent Warming",
       subtitle = "Recent Rate 3x 40-Year Average",
       x = "", 
       y = "Sea Surface Temperature Anomaly",
       caption = paste0("Anomalies calculated using 1982-2011 reference period.")) +
  # theme
