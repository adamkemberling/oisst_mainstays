...
# Add Logo
# add_gmri_logo(ersst_long_term_p, position = "bot_left")
...
record_percents <- region_hw %>% 
  ...
  #scale_alpha_manual(values = c("100% Record Temp." = 1, "filler" = 0.6)) +
...
```{r, eval = FALSE}
# Making the line blue whenever it goes under:


# Temps in March less than thresh
#this_yr %>% filter(month(time) == 3)
#under_thresh <- this_yr %>% filter(sst_f < mhw_thresh_f)
# Join back to dates to respect NA's
# under_thresh <- this_yr %>% select(time) %>% left_join(under_thresh)

# ggplot(this_yr, aes(x = time)) +
#     geom_segment(aes(x = time, xend = time, 
#                      y = seas_f, yend = sst_f), 
#                  color = "#0571B0", alpha = 0.25) +
#     geom_segment(aes(x = time, xend = time, 
...
#     geom_line(aes(y = hwe_f, color = "Heatwave Event")) +
#     geom_line(aes(y = mhw_thresh_f, color = "MHW Threshold"), lty = 3, size = .5) +
#     geom_line(data = under_thresh, aes(time, sst_f, color = "Sea Surface Temperature")) +
#     geom_textpath(aes(y = seas_f), color = "gray30", label = "Climatological Mean", hjust = 0.5, lty = 2) +
#     scale_color_manual(values = color_vals) +
#     scale_x_date(date_labels = "%b", date_breaks = "1 month", expand = expansion(mult = c(0,0))) +
#     scale_y_continuous(labels =  number_format(suffix = " \u00b0F")) +
...
#add_gmri_logo(year_timeline, position = "top_left")

# Now add logos to save individually
#hw_temp_p <- add_gmri_logo(hw_temp_p, position = "top_left")
#hw_anom_p <- add_gmri_logo(hw_anom_p, position = "top_left") 
