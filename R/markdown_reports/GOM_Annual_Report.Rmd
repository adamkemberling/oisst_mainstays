...
# Benchmark the averags temp and maximum temps
...
# Plot them
ggplot(data = ersst_yr) +
  geom_line(aes(yr, sst),
            group = 1, alpha = 0.4, linetype = 1, color = gmri_cols("gmri blue")) +
  geom_point(aes(yr, sst), 
             size = 1, color = gmri_cols("gmri blue")) +
  geom_point(data = filter(ersst_yr, sst > long_max), aes(yr, sst), color = "darkred", size = 1.5) +
  geom_mark_ellipse(aes(yr, sst, filter = sst > long_max,
  ...
  geom_mark_ellipse(aes(yr, sst, filter = sst == long_max, label = yr, description = "Former Hottest Temperature on Record"), 
                    label.fontsize = 9,  label.buffer = unit(0.25, "lines"),
                    color = "darkred", linetype = 2) +
  scale_color_gmri() +
  theme_gmri() +
  scale_x_continuous(expand = expansion(add = c(2,10))) +
  scale_y_continuous(expand = expansion(add = c(0.5,3)),
    limits = c(),
    sec.axis = sec_axis(trans = ~as_fahrenheit(., data_type = "temperature"),
...
Because the ERSST data is of a coarser resolution (monthly measurements at a 0.5 x 0.5 degree grid) and does not capture the warmer inshore dynamics (a bias to show colder temperature). The OISSTv2 temperatures in this area are on average warmer. **Across the years where data from both sources is available, ERSSTv5 is on average `r round(temp_difference,2)`$^oC$ Colder than OISSTv2 data.** 
...
# # Too many things on one plot
# 
# ####  Annual Trend Plot  ####
# ggplot(data = annual_summary, aes(yr_as_dtime, area_wtd_anom)) +
#   
#   # Overlay yearly means
#   geom_line(color = "gray10", size = 0.75, linetype = 3) +
#   geom_point(color = "gray10", size = 1) +
#   
#   # Add regression lines
#   geom_labelsmooth(
#     data = filter(global_summary, year <= 2021),
#     method = "lm", 
#     formula = y ~ x, se = F,
#     label = "1982-2021 Global",
...
#     hjust = 0.9, alpha = 0.8) +
#   geom_labelsmooth(
#     data = filter(annual_summary, year <= 2021),
#     method = "lm",
#     formula = y ~ x, se = F,
#     aes(label = "1982-2021 Gulf of Maine"), #label = "40-Year Trend",
#     size = 3,
#     linewidth = 1,
#     label.padding = unit(0.075, "lines"),
#     linetype = 1,
...
#   #   data = filter(annual_summary, year %in% c(2007:2021)),
#   #   method = "lm", 
#   #   aes(label = "2007-2021 Gulf of Maine"),
...
#             aes(label = "Warming Rates:", 
#                 x = min(region_timeseries$time), y = Inf),
#             hjust = -0.25, vjust = 7, color = "black") +
#   geom_text(data = data.frame(), 
#             aes(label = eq_global, x = min(region_timeseries$time), y = Inf),
#             hjust = -0.25, vjust = 8.5, color = gmri_cols("green")) +
#   geom_text(data = data.frame(),
#             aes(label = eq_all, x = min(region_timeseries$time), y = Inf),
#             hjust = -0.25, vjust = 10, color = gmri_cols("gmri blue")) +
#   # geom_text(data = data.frame(), 
#   #           aes(label = eq_15, x = min(region_timeseries$time), y = Inf),
#   #           hjust = -0.25, vjust = 10, color = gmri_cols("orange")) +
#   
#   # Axes
#   #scale_x_continuous(expand = expansion(add = c(2,10))) +
#   scale_y_continuous(
...
#        subtitle = "Recent Rate 4 x Global Average",
#        x = "", 
...
gom_all_trend <- ggplot(data = annual_summary, aes(year, area_wtd_anom)) +
  ...
  # Manually add equations so they show yearly not daily coefficients
  geom_text(data = data.frame(), 
            aes(label = "Warming Rate:", 
                x = 1982, y = Inf),
            hjust = -0.25, vjust = 2.25, color = "black") +
  geom_text(data = data.frame(),
            aes(label = eq_all, x = 1982, y = Inf),
            hjust = -0.25, vjust = 3.75, color = gmri_cols("gmri blue")) +
...
```{r gom recent warming}
# New Plot
# Gulf of Maine Warming
gom_recent_trend <- ggplot(data = annual_summary, aes(year, area_wtd_anom)) +
  
  # Overlay yearly means
  geom_line(color = "gray10", size = 0.75, linetype = 3, group = 1) +
  geom_point(color = "gray10", size = 1) +
  ...
  geom_text(data = data.frame(), 
            ...
            hjust = -0.25, vjust = 2.25, color = "black") +
  geom_text(data = data.frame(),
            aes(label = eq_15, x = 1982, y = Inf),
            hjust = -0.25, vjust = 3.75, color = gmri_cols("orange")) +
...
# Overlay yearly means
glob_trend <- ggplot(data = filter(global_summary, between(year, 1982, 2021)), 
       aes(year, area_wtd_anom)) +
  geom_line(color = "gray10", size = 0.75, linetype = 3) +
  geom_point(color = "gray10", size = 1) +
  # Add regression lines
  geom_smooth(
  ...
  geom_text(data = data.frame(), 
            ...
            hjust = -0.25, vjust = 2.25, color = "black") +
  geom_text(data = data.frame(), 
            aes(label = eq_global, x = 1982, y = Inf),
            hjust = -0.25, vjust = 3.75, color = gmri_cols("green")) +
  ...
  theme_gmri() +
  # labels + theme
  labs(title = "Global Oceans: Long-Term Warming Trend",
       x = "Year", 
       y = "SST Anomaly",
       caption = paste0("Anomalies calculated using 1982-2011 reference period."))
...
gom_all_trend / gom_recent_trend / glob_trend
```
...
annual_summary %>% 
  ...
  ggplot(aes(yr_as_dtime, area_wtd_anom)) +
...
world_ranks_binned <- world_rank_stars %>% 
  mutate(
    ...
    rank_bin = cut(rank, breaks = seq(0.1, 1, by = 0.1))) %>% 
...
global_ranks_zoom <- ggplot() +
  ...
  geom_sf(data = new_england, color = "white", fill = "gray30", size = .075) +
  geom_sf(data = canada, color = "white", fill = "gray30", size = .075) +
  geom_sf(data = greenland, color = "white", fill = "gray30", size = .075) +
  scale_y_continuous(breaks = seq(-90, 90, by = 5)) +
  scale_x_continuous(breaks = seq(-180, 180, by = 10))+
  scale_fill_brewer(palette = "YlOrRd", direction = 1, na.value = "transparent") +
  map_theme +
  coord_sf(
...
global_ranks <- ggplot() +
  ...
  guides("fill" = guide_colorsteps(title = expression("Global Percentile of Warming Rates"),
                                 title.position = "top", 
                                 title.hjust = 0.5,
                                 barwidth = unit(2.5, "in"),
                                 frame.colour = "black",
