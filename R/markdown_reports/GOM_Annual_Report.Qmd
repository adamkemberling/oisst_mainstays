...
```{r, eval = F}
# Climatology figure for something random
report_yr_only %>% 
  mutate(time = as.Date(time),
         mcs_thresh_f = as_fahrenheit(mcs_thresh, data_type = "temperature")) %>% 
ggplot(aes(time)) +
  geom_ribbon(aes(ymin = as_fahrenheit(mcs_thresh), ymax = mhw_thresh_f), fill = "lightblue") +
  geom_line(aes(y = seas_f), color = gmri_cols("blue"), linetype = 1, linewidth = 2) +
  scale_x_date(date_breaks = "1 month", labels = date_format("%b"),
               expand = expansion(add = c(0,0))) +
  scale_y_continuous(labels = label_number(suffix = deg_f)) +
  labs(x = "Day of the Year", y = "Sea Surface Temperature",
       title = "Gulf of Maine's Seasonal Climate ")
...
# # Saving the plot as SVg for Scott
# temps_svg <- global_rate_comparison(
#   annual_summary_dat = annual_summary, 
#   global_summary_dat = global_summary, 
#   eq_all = str_remove(eq_all_f, "1982-2023"), 
#   eq_global = str_remove(eq_global_f, "1982-2023"), 
...
#   region_label = "Gulf of Maine")  +
#   coord_cartesian(clip = "off") +
#   theme(legend.position = c(0.2, -0.175))
# 
# # Remove the points from the plot
# temps_svg$layers[[2]] <- NULL
# 
# 
# ggsave(plot = temps_svg, filename = here::here("local_data/GulfofMaine_warming_rates.svg"), 
#        height = 4, 
#        width = 6,
#        units = "in", scale = 1.5)
```
...
#| fig-cap: "Temperature anomaly horizons for all years of available satellite data (1982-2023). Horizons display how long temperatures were above certain thresholds (horizon width) and are colored by the strength of these events."
