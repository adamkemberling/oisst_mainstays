...
# some acknowledgement of the extreme characteristics:
```



```{r}
#| label: all-year-summary
...
temp_simplified <- global_rate_comparison(
  ...
  geom_logo(x_npc = 0.12, y_npc = -0.135, logo_height = 1, height_units = "cm")
...
The rate of warming in the region vary throughout the year. Comparisons across the four seasons @fig-seasons-simplified reveal that the Gulf of Maine is warming fastest during the Summer and Fall months (~4x the global average), nearly twice the rate seen during the spring (~2x the global average).


```{r}
#| label: fig-seasons-simplified
...
season_panels <- season_avgs %>% 
  split(.$season_eng) %>% 
  map(function(x){
    ggplot(x, aes(season_yr, anom_f)) +
      geom_line( data = filter(x, Region == "Gulf of Maine"), linewidth = .5) +
      geom_point(data = filter(x, Region == "Gulf of Maine"), size = .5) +
      geom_smooth(
        aes(color = rate_eq), 
        method = "lm", 
        formula = y~x, linetype = 1, linewidth = 1.5, se = FALSE) +
...
```{r}
#| label: fig-new-england-annual-map
...
When we compare `r report_yr` to other unusually warm years in the Gulf of Maine, we see that in absolute terms (the top stack of bars in the figure below), the difference between the warmest five years may not seem so large, but when we look at the deviation from the long-term average SST (i.e., the annual SST anomaly), `r report_yr` really pops out, putting a finer point on just how unusually warm the Gulf of Maine was this past year.

```{r}
#| label: fig-yr-ranks
#| fig-cap: "Bar plots of absolute sea surface temperature (SST) values (top panel) and SST anomalies (bottom panel) for the top five warmest years in the Gulf of Maine."
#| fig-height: 8

# Plot the metrics of the hottest years:
hottest_yr_panels <- yearly_metric_plots(
  temp_dat = region_hw, 
  temp_units = "F", 
...
# Display plot
hottest_yr_panels
grid::grid.raster(lab_logo, x = 0.125, y = 0.08, just = c('left', 'bottom'), width = unit(0.8, 'inches'))
```
...
#| fig-cap: "A bar plot showing the percentage of days during each month in 2021 when a record-high temperature was observed in the Gulf of Maine (e.g., 100% — or every day — of October 2021 experienced a record-setting high temperature)."
...
record_percents <- region_hw %>% 
  ...
  labs(x = "Month", 
       ...
       subtitle = "Large fraction of Nov. & Dec. dates were Gulf of Maine all-time records")   +
  coord_cartesian(clip = "off") +
...
comparison_horizons <- hori_2021 / hori_report_yr + plot_layout(guides = "collect")
...
hw_streamplot <- ggplot(day_types, aes(yr, y = day_total, fill = `Anomaly Strength`)) +
  ...
  theme(
   ...
   guides(fill = guide_legend(title = "Temperature Anomaly \u00b0F",
                             ...
                             byrow = T)) +
  coord_cartesian(clip = "off") +
  geom_logo(x_npc = 0.1, y_npc = -0.125, logo_height = 1, height_units = "cm")
...
# Yearly HW Metrics
panels_c <- yearly_metric_plots(temp_dat = region_hw, 
                                temp_units = "C", 
                                year_focus = report_yr)
...
hw_temp_p_c <- year_hw_temps(year_hw_dat = report_yr_only, temp_units = "C")
...
# heatmap
heatmap_c <- heatwave_heatmap_plot(hw_dat = region_hw, temp_units = "C")
