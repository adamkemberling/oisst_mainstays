...
global_anoms <- read_csv(
  ...
  supplement_season_info()  %>% 
  # Change column names to favor the area-weighted values
  rename(
    sst = unweighted_sst,
    sst_anom = unweighted_sst_anom,
    area_wtd_sst = area_weighted_sst,
    area_wtd_anom = area_weighted_sst_anom) 
...
month_summ_table <- monthly_avgs %>% 
  ...
  cols_label( # Change the column names to the units
    rank = md("Warm to Cold Ranking (1982-2024)"),
...
hw_anom_p <- year_hw_dat %>% 
  ...
  geom_ribbon(aes(x = time, ymin = cs_anom_thresh, ymax = hw_anom_thresh), fill = "lightgray", alpha = 0.4) +
    ...
    geom_textpath(
      ...
      hjust = 0.5, lty = 1, family = "Avenir", vjust = 1.2, size = 4) +
    geom_bracket(
      data = hw_events, 
...
# Turn it on its side, use a line instead of points
heatmap_horizontal <- region_hw %>% 
  mutate(hw_date = if_else(mhw_event, flat_date, NA)) %>% 
  ggplot(aes(yr, flat_date)) +
  geom_tile(aes(fill = anom_f)) +
  scale_fill_distiller(
    palette = "RdBu", 
    limits = c(-5,5),
    labels = label_number(suffix = deg_f), oob = oob_squish) +
  geom_line(aes(x = yr, y = hw_date, group = str_c(mhw_event_no, yr)), 
            color = "white", linewidth = 1)+
  geom_line(aes(x = yr, y = hw_date, group = str_c(mhw_event_no, yr)), 
            color = "black", linewidth = 0.6) + 
  scale_x_continuous(limits = c(1982 + .5, params$report_year + .5), expand = expansion(add = c(0,0))) +
  scale_y_continuous(
    trans = c("date", "reverse2"),
    labels = label_date(format = "%b"),
    breaks = seq.Date(from = min(region_hw$flat_date), max(region_hw$flat_date), by = "1 month"), 
    expand = expansion(add = c(0,0))) +
  # scale_y_date(date_labels = "%b", date_breaks = "1 month", expand = expansion(add = c(0,0))) +
  guides(
    "fill" = guide_colorbar(
      title = "Sea Surface Temperature Anomaly", 
      title.position = "top", 
  ...
  theme(legend.position = "bottom") +
  labs(
    title = "Gulf of Maine SST Anomalies and Marine Heatwave Record",
    x = "Year",
    y = "Month")
  

# Now below that, plot the MHW Days and separate events
events_plot <- region_hw %>%
  filter(yr >= 1982) %>% 
  group_by(yr, mhw_event_no, mcs_event_no) %>% 
  summarise(
    mean_anom = mean(anom_f, na.rm = T),
    hwdays = sum(mhw_event|mcs_event, na.rm = T)) %>% 
  
  ggplot() +
  geom_col(aes(yr, hwdays, fill = mean_anom), 
           color = "white", position = "stack", show.legend = F) +
  scale_fill_distiller(
    palette = "RdBu", 
    limits = c(-5,5),
    labels = label_number(suffix = deg_f), oob = oob_squish) +
  # guides(
  #   "fill" = guide_colorbar(
  #     title = "Average Event Surface Temperature Anomaly", 
  #     title.position = "top", 
  #     title.hjust = 0.5,
  #     barwidth = unit(3.5, "inches"), 
  #     frame.colour = "black", 
  ...
  scale_x_continuous(limits = c(1982 + .5, params$report_year + .5), expand = expansion(add = c(0,0))) +
  labs(x = "Year", y = "Extreme Event Days", title= "Heatwave and Cold Spell Events")



(
  (heatmap_horizontal + theme(plot.margin = margin(1,1,2,1)))/ 
    (events_plot + theme(plot.margin = margin(2,1,1,1)))
...
# Do the same for global temperatures
...
```{r}
#| label: New England 2021 Map C
#| eval: false


# Set time limits to get data
annual_dates_21 <- as.Date(
...
# Process the yearly average
ne_annual_anom <- mean(ne_anoms, na.rm = T)

# Make it a stars class to plot with ggplot
ne_anoms_st <- st_as_stars(ne_annual_anom)
...
annual_gom_map <- ggplot() +
  ...
  scale_fill_distiller(
    ...
    oob      = scales::oob_squish) +
  coord_sf(xlim = c(-78, -56.5), ylim = c(37.8, 48)) +
  labs(title = "2024: Average Temperature Anomalies",
       caption = "Anomalies calculated using 1991-2020 climatology.\n100m & 200m contours overlaid to show bathymetric details.") +
  map_theme(
    legend.position = "bottom",
    legend.title = element_text(angle = 0, face = "bold", family = "Avenir"))+
...
By June and July, the separation between the region's SSTs and the previous records had widened by over a degree, creating a dramatic and alarming figure that was shared widely on social media. @fig-natl-timeseries captures how profoundly and consistently hot this year's temperatures were across the North Atlantic. 
...
# natl_ts <- read_csv(here("notebooks/nb_testing_data/north_atlantic_sst_1982to2025.csv"))

# Changed path to put on box
natl_ts_path <- str_c(oisst_path, "regional_timeseries/gmri_sst_focal_areas/OISSTv2_north_atlantic.csv")
natl_ts <- read_csv(natl_ts_path)
...
natl_25 <- filter(natl_ts, yr == 2025)
...
north_atl_plot <- natl_23 %>% 
  ...
  geom_line(
    ...
    aes(group = yr, color = factor(yr),
        linewidth = I(record_temp_lw),
        alpha = I(record_temp_alpha)),
    show.legend = F) +
  # # 2025
  # geom_line(data = natl_25, aes(flat_date, anom_f, color = "2025")) +
  # Try to fix how blocky ggplot lines are
  # geom_line(
  #   data = mutate(natl_record_yrs, anom_f = if_else(is_record, anom_f, NA)),
  #   aes(group = yr, color = factor(yr), 
  #       alpha = I(record_temp_alpha)), 
  #   linewidth = 1,
  #   show.legend = F) +
  # geom_line(
  #   data = mutate(natl_record_yrs, anom_f = if_else(is_record, NA, anom_f)),
  #   aes(group = yr, color = factor(yr), 
  #       alpha = I(record_temp_alpha)), 
  #   linewidth = 0.5,
  #   show.legend = F) +
...
scale_color_gmri() +
  ...
  labs(x = "Day of Year", y = "Sea Surface Temperature Anomaly",
       title = str_c("Record Warm Daily SSTs on ", sum(natl_24$is_record)," Days in North Atlantic"),
       subtitle = "(0-60N, 0-80W)")  +
  coord_cartesian(clip = "off") +
  
  theme(text = element_text(family = "Avenir")) +
  geom_logo(x_npc = 0.85, y_npc = -.175, logo_height = 1, height_units = "cm")
...
global_anoms_stack <- stack(
  str_c(oisst_path, "yearly_averages/annual_sst_1982to2024.nc"), 
  varname = "sst_anom")
...
```{r}
#| label: Load Buoy M timeseries

# Read the data we downloaded from erddap
buoy_m <- read_csv(here::here("local_data/buoy_m_250m.csv")) %>% 
  mutate(temperature = as_fahrenheit(temperature))
...
# Get Monthly SST
sst_monthly <- region_hw %>% 
  group_by(yr, month = month(time)) %>% 
  summarise(
    across(c(sst_f, seas_f, anom_f), \(x)mean(x, na.rm = T)), 
    .groups = "drop") %>% 
  mutate(time = as.Date(
    str_c(
      yr, 
      str_pad(month, pad = "0", side = "left", width = 2),
      "01", sep = "-")),
    anom_col = if_else(anom_f > 0, "#B2182B", "#2166AC"))
...
# Plot monthly as a line, and color it on whether its abov/below monthly mean
bm_monthly %>% 
  mutate(anom_col = if_else(is_pos, "#B2182B", "#2166AC")) %>% 
  ggplot() +
  geom_line(aes(time, temperature, color = I(anom_col), group = 1)) +
  # geom_smooth(
  #   aes(time, temperature), 
  #   formula = y ~ x,
  #   method = "lm",
  #   color = "black") +
  labs(title = "Buoy M: Temperature at 250 meters",
       y = "Temperature",
       x = "Date")
```
...
# Annotations
ocean_labs<- tribble(
~"label",             ~"lat",~"lon",~"angle",~"size",
"Georges\nBank",        41.15,  -67.75, 0, 4,
# "Jordan\nBasin",       43.5,   -67.5,  0, 4,      
# "Wilkinson\nBasin",    42.29,  -69.2,  0, 4,   
# "Northeast\nChannel",    42.385, -66.2,  0,  4,
"Scotian\nShelf",      43.675, -63.2,  0, 4,
"Mid-Atlantic\nBight", 40,     -72.6,  0,  4,
"Grand\nBanks",      45,     -51,   0, 4#,
# "Laurentian Channel",  46.3,   -58.2,  310, 4#,
#"Nantucket\nShoals",      41.5,   -68.5, 0, 4
)
...
buoy_map <- 
  ...
  geom_point(
    data = buoy_locations, 
    aes(label_lon, label_lat), 
    shape = 21, fill = "orange", size = 2) +
  # ggforce::geom_mark_ellipse(
  #   data = buoy_locations,
  #   aes(label_lon, label_lat, label = Buoy, description = "Jordan Basin"),
  #   colour = "black", label.family = "Avenir", 
  #   label.fontsize = 16, 
  #   label.buffer = unit(1.5, "cm")) +
  geom_mark_ellipse(
    data = buoy_locations,
    aes(label_lon, label_lat),
    colour = "black") +
  geom_shadowtext(
    ...
    size = 3, fontfamily = "Avenir", color = "black", bg.colour = "white", lineheight = 0.75) +
  # coord_sf(xlim = c(-78, -56.5), ylim = c(37.8, 48)) + # annual map view
  ...
  labs(
    ...
    title = "Deep Water Oceanographic Buoy",
    subtitle = "Buoy M, Jordan Basin")
...
#| fig-height: 8
#| fig-width: 8
#| label: buoy timeseries and map


# three levels: map, sst monthly, bt monthly, aligned x axis


# Plot the Surface Temperature Anomalies on Monthly Scale
monthly_sst_plot <- sst_monthly %>% 
  ggplot() +
  geom_line(aes(time, 0, group = 1)) +
  geom_line(aes(time, anom_f, color = I(anom_col), group = 1)) +
  geom_segment(
    aes(time, 0, xend = time, yend = anom_f, group = 1, color = I(anom_col)), alpha = 0.3) +
  geom_line(aes(time, anom_f, color = I(anom_col), group = 1)) +
  scale_y_continuous(labels = label_number(suffix = deg_f)) +
  theme(plot.margin = margin(t = 1,r = 1,l = 2,b = 1))+
  labs(
    y = "Temperature Anomaly",
    x = "Date",
    title = "Sea Surface Temperature",
    subtitle = "NOAA OISSTv2 Sea Surface Temperature")



# Plot monthly as a line, and color it on whether its abov/below monthly mean
buoy_anoms_plot <- sst_monthly %>% 
  distinct(time) %>% 
  left_join(bm_monthly) %>% 
  ggplot() +
  geom_line(aes(time, 0, group = 1)) +
...
# Make the plot
buoy_triple <- (buoy_map) | 
  (( buoy_anoms_plot / monthly_sst_plot) + plot_layout(axes = "collect"))
buoy_triple
...
glorys_bot_mhw %>% 
  ...
  geom_ribbon(
    ...
    fill = "lightgray", alpha = 0.4) +     
  geom_path(
    aes(x = time, y = bt_f, color = mhw_event_txt, group = 1), 
  ...
  labs(title = "Gulf of Maine Bottom Temperature",
       ...
       color = "Bottom Temperature and Heatwave Status",
...
glorys_bot_mhw %>% 
  ...
  geom_textpath(
      ...
      color = "gray10", 
      label = "Climatological Avg.", 
      hjust = 0.85, lty = 1, family = "Avenir", vjust = 1.2, size = 4) +
  scale_color_manual(values = color_vals) +
  scale_y_continuous(
    labels =  number_format(suffix = temp_ops$temp_suff),
    expand = expansion(mult = c(0.05, .2))) +   
...
```{r}
#| label: Glorys gulf of maine sst anom timeseries
#| eval: false 
#| fig-height: 5

# # Prepare MHW and climatology assesment for surface and bottom temperatures
# glorys_surf_mhw <- pull_heatwave_events(
#   temperature_timeseries = as.data.frame(glorys_ts), 
#   threshold = 90, 
#   clim_ref_period = c("1993-01-01", "2022-12-31"), 
...
#             linewidth = 0.8, key_glyph = "timeseries") +
#   theme(legend.position = "bottom", legend.title.position = "top") +
#   labs(title = "Gulf of Maine Surface Temperature Anomalies",
#        x = "Date",
#        y = "Surface Temperature",
#        color = "Surface Temperature Anomalies and Heatwave Status",
#        caption = "Surface Temperature from GLORYS Ocean Reanalysis by Copernicus Marine Service")
...
bt_23_quarter_map <- ggplot() +
  ...
  labs(title = str_c("2023: Average Temperature Anomalies"),
       caption = "Anomalies calculated using 1993-2022 climatology.\n100m & 200m contours overlaid to show bathymetric details.") +
  map_theme(
    legend.position = "bottom",
    legend.title = element_text(angle = 0, face = "bold", family = "Avenir"))+
  guides("fill" = guide_colorbar(
...
# Obtained the data directly from Ben Tupper
# Load the North wall position from the Navy's Data
...
navy_gsi %>% 
  ...
  scale_fill_brewer(
    palette = "Blues",
    na.value = "#08519C")  +
  geom_sf(aes(color = yr, alpha = I(flag_alpha))) +
  scale_color_manual(values = c("gray80", "gray20")) +
  #coord_sf(xlim = c(-79, -56.5), ylim = c(35, 48)) +
  coord_sf(xlim = c(-74.5, -49.5), ylim = c(37, 48)) + # full zoom out
  theme(legend.position = "bottom", legend.title.position = "top") +
...
# # Testing:
# resample_line(line = nwall[1,], n_points = 200) %>% 
#     st_as_sf(crs = 32619) %>% 
#     rename(geometry = x) %>% 
#     st_cast("POINT") %>%
#     mutate(point_id = row_number())
# nwall %>% filter(st_is_empty(nwall) == T)
# nwall %>% filter()

# Interpolate each daily line into `n_points`
# Group  by point id
resampled_points <- nwall %>%
  filter(st_is_empty(nwall) == F) %>% 
  split(.$date) %>% 
  map_dfr(
    ~resample_line(
      line = .x, 
...
monthly_avg_lines %>% 
  ...
  geom_sf(data = new_england) +
  geom_sf(data = canada) +
  geom_sf(alpha = 0.8, linewidth = 0.8) +
  facet_grid(mnth~yr) +
...
if(params$save_figures){
  ...
  # Buoy Temperatures Trio
  ggsave(
    plot = buoy_triple, 
    filename = str_c(save_location, "deepwater_temperatures_plot.jpeg"),
    height = unit(2.5, "in"),
    width = unit(4, "in"),
    dpi = "retina", 
    bg = "white", 
