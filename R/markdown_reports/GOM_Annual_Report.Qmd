...
temp_simplified <- temp_simplified +
  annotation_custom(grob = leg2, xmin = 2009, xmax = 2021, ymin = -1.3, ymax = -1)
temp_simplified
...
season_panels <- season_avgs %>% 
  ...
  imap(function(x, y){
    ...
    season_p <- ggplot(x, aes(yr, anom_f)) +
      geom_line( data = filter(x, Region == "Gulf of Maine"),alpha = 0.5, linewidth = .5, linetype = 3) +
      geom_point(data = filter(x, Region == "Gulf of Maine"), size = 2, alpha = 0.8) +
      stat_smooth(
        aes(color = rate_eq), 
        method = "lm", 
        formula = y~x, 
