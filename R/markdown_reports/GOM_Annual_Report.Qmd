 ...
 * The most extreme temperatures in 2024 occurred in February and March, each setting new records for highest monthly average SST in the Gulf of Maine.

 * The “coolest” monthly average temperature anomalies for 2024 occurred during September, November, and December. Each of last five months this year had SSTs below the top ten warmest.
 
 * SSTs in 2024 were consistent with the long-term trend of increasingly warm conditions driven primarily by anthropogenic climate change. 
...
## 2024: SSTs Across the North Atlantic
...
region_hw <- region_hw %>% 
  mutate(
    is_ryear = ifelse(yr == params$report_year, TRUE, FALSE),
    yr_col   = ifelse(is_ryear, gmri_cols("lv orange"), "gray90"),
    yr_lw    = ifelse(is_ryear, 1.5, 0.8),
    yr_alpha = ifelse(is_ryear, 1, 0.5),
    flat_date = as.Date("2000-01-01") + doy-1,
...
gom_ryear <- region_hw %>% 
  filter(yr == params$report_year) %>% 
  mutate(
    is_record = time %in% hottest_days$time,
    record_temp_alpha = ifelse(is_record, 1, 0.4),
    record_temp_lw = ifelse(is_record, 1.2, 0.7))
record_days <- sum(distinct(gom_ryear,time, .keep_all = T)$is_record)
...
gom_ryear_record_ts <- gom_ryear %>% 
  ...
  geom_texthline(
    ...
    aes(yintercept = yintercept), 
    label = "1991-2020 Climatological Average", 
    linewidth = 1, size = 4, family = "Avenir", hjust = 0.5, vjust = 1.1) +
  geom_line(
    aes(group = yr, linewidth = I(record_temp_lw), 
        alpha = I(record_temp_alpha), color = I(yr_col))) +
  geom_text(
    data = gom_ryear %>% slice(yday(min(gom_ryear$time)) + 150),
    label = "Record High SSTs", vjust = -3.5, size = 4,
    fontface = "bold", color = gmri_cols("lv orange"), family = "Avenir") +
  scale_x_date(date_breaks = "1 month", labels = date_format("%b"), 
               expand = expansion(add = c(0,0))) +
  annotate(
    'curve',
    x = as.Date("2000-01-01") + 146, # Play around with the coordinates until you're satisfied
    y = 5.4,
    yend = 4.2,
    xend = as.Date("2000-01-01") + 156,
    linewidth = 0.5,
    curvature = -0.5,
  ...
  labs(x = "Day of Year", y = "Sea Surface Temperature Anomaly",
       title = str_c("Record Warm SST for ", record_days, " Days of 2024 in the Gulf of Maine"),
...
#          lat = as.numeric(lat)) %>%
#   filter(season == 2024,
#          between(iso_time, as.POSIXct('2024-07-01 00:00:00'), as.POSIXct('2024-11-30 00:00:00')),
#          name == "LEE",
#          between(lon, crop_x_expanded[[1]], crop_x_expanded[[2]])) %>%
#   arrange(iso_time)
# 
# # Save it so we don't need to load all of them:
# write_csv(lee_track, here::here("local_data/hurricanes/lee_2024.csv"))

# # Load Hurricane Lee
# lee_track <- read_csv(here::here("local_data/hurricanes/lee_2024.csv"))
# 
# 
# 
# 
...
### Below Average SST Temperature Days
