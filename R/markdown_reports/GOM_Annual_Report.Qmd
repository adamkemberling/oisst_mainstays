...
# Can't add logo
ersst_v_oisst # + add_gmri_logo(ersst_v_oisst, position = "bot_left", layer = 4)
grid::grid.raster(lab_logo, x = 0.05, y = 0.03, just = c('left', 'bottom'), width = unit(0.8, 'inches'))
```
...
2021 was the hottest year on record for the Gulf of Maine, shifting the 1982-2021 warming rate to `r str_c(rate_data[["GoM All F"]][["decadal_rate"]], "\u00b0F / Decade")`. 
...
# Display plot
#annual_temps_plot <- add_gmri_logo(annual_temps_plot, position = "top_left", nudge_y = .35)
annual_temps_plot
grid::grid.raster(lab_logo, x = 0.05, y = 0.03, just = c('left', 'bottom'), width = unit(0.8, 'inches'))
```
...
#gom_anomaly_bars <- add_gmri_logo(gom_anomaly_bars, position = "bot_left")
...
#        caption = "Temperature data covers period of 1982-2021")  +
#    theme(legend.position = c(0.25, 0.85),
#          legend.background = element_rect(color = "black", fill = "white")) +
#   guides(color = guide_legend(title.position = "top", title.hjust = 0.5))
# 
...
hw_temp_p <- year_hw_temps(year_hw_dat = this_yr, temp_units = "F")
...
During 2021 there were `r events_num` marine heatwave event(s),  lasting a total of `r days_hw`. These events include one that carried over from the previous year and another that has persisted into 2022. The average duration for these events was `r avg_duration` days (excluding days in 2020 & 2022).
...
hw_anom_p <- year_hw_anoms(year_hw_dat = this_yr, temp_units = "F")
hw_anom_p
grid::grid.raster(lab_logo, x = 0.05, y = 0.03, just = c('left', 'bottom'), width = unit(0.8, 'inches'))
...
# Assemble pieces
heatwave_heatmap
#add_gmri_logo(heatwave_heatmap, position = "bot_left")
grid::grid.raster(lab_logo, x = 0.05, y = 0.03, just = c('left', 'bottom'), width = unit(0.8, 'inches'))
```
...
gom_annual_horizons <- anom_horizon_plot(
    grid_data = filter(grid_data, yr <= 2021), 
    origin = ori, 
    scale_cutpoints = sca) +
...
# Plot it
gom_annual_horizons
# add_gmri_logo(gom_annual_horizons, "bot_left")
grid::grid.raster(lab_logo, x = 0.05, y = 0.03, just = c('left', 'bottom'), width = unit(0.8, 'inches'))
...
# Show figure
hw_streamplot
# add_gmri_logo(hw_streamplot, "bot_left")
grid::grid.raster(lab_logo, x = 0.08, y = 0.04, just = c('left', 'bottom'), width = unit(0.8, 'inches'))
```
...
if(params$save_figures){
  ...
  ggsave(plot = record_percents,
         filename = str_c(save_location, "2021_percent_record_bars.png"),
         dpi = "retina", bg = "white")
