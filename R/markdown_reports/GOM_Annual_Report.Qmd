...
## 2023: Alarming Conditions Across the North Atlantic
...
natl_23 %>% 
  ...
  geom_text(
    ...
    label = "2020", vjust = -3, size = 5, fontface = "bold", 
  ...
  geom_line(
   ...
   geom_text(
    data = natl_22 %>% slice(yday(min(natl_ts$time))+30),
    label = "2022", vjust = -3, size = 5, fontface = "bold", 
  ...
  geom_logo(x_npc = 0.875, y_npc = -.13, logo_height = 1, height_units = "cm")
...
## 2023 Sea Surface Temperature in the Gulf of Maine
...
#| fig-cap: "Sea surface temperature anomalies (orange) and record daily SSTs (orange-bold) for the Gulf of Maine in 2023. The 1991-2020 average daily SST (black) and 10th - 90th percentile range is shown in gray."
...
gom_23_record_ts <- gom_23 %>% 
  ...
  geom_line(aes(group = yr, linewidth = I(record_temp_lw), alpha = I(record_temp_alpha), 
                color = I(yr_col))) +
  geom_text(
    data = gom_23 %>% slice(yday(min(gom_23$time))+50),
    label = "2023", vjust = -3, size = 5,
    fontface = "bold", color = gmri_cols("orange"), family = "Avenir") +
  scale_x_date(date_breaks = "1 month", labels = date_format("%b"), 
               expand = expansion(add = c(0,0))) +
...
lee_map <- ggplot() +
  ...
  geom_point(data = lee_track, aes(lon, lat), 
            color = gmri_cols("orange"), alpha = 0.8, size = 2) +
  geom_path(data = lee_track, aes(lon, lat, linetype = nature), 
            color = gmri_cols("orange"), alpha = 0.8, linewidth = 1) +
  ...
  labs(x = NULL, y = NULL, title = "Passage of Extratropical Storm Lee", linetype = "Storm Strength")

# show map
lee_map


# # Grab the temps from 5 days prior to five days after
# # 9/10 - 9/30
# 
# lee_temps <- region_hw %>% filter(between(time, as.Date("2023-09-10"), as.Date("2023-09-25")))
# 
# ggplot(lee_temps) +
#   geom_ribbon(aes(ymin = cs_thresh_f, ymax = hw_thresh_f, x = time), fill = "lightgray", alpha = 0.4) +
#   geom_hline(yintercept = 0) +
#   #geom_line(aes(time, anom_f, group = 1)) +
#   geom_line(aes(x = time, y = anom_f, group = yr, color = status)) +
#   labs(y = "Sea Surface Temperature Anomaly")
...
temp_simplified <- temp_simplified +
  annotation_custom(grob = leg2, xmin = 2009, xmax = 2020, ymin = -2.5, ymax = -1.5)
temp_simplified
...
clim_labels_df <- year_hw_dat %>% 
  ...
  mutate(
    ...
    y = case_when(
      labs == hw_thresh_col  ~ y+0.5,
      labs == clim_col       ~ y+0,
      labs == cs_thresh_col  ~ y-0.5),
    hjust = 5+time)
...
#| fig-height: 5
#| eval: true
#| fig-cap: "A timeseries of marine heatwave (MHW) conditions in the Gulf of Maine extending from January 1, 2023 through August  31, 2023. Black lines represent the long-term (i.e., 1991 – 2020) average SST, the 10th percentile, and 90th percentile for a given day in the Gulf of Maine; a solid line (red for marine heatwave or blue for a non-event) indicate the observed SST this year; red (above 90th percentile) and blue (below 90th percentile) shading illustrates how far the observed SST autumns from the MHW threshold."
...
### Above Average and Record Setting Temperatures
...
###  The Decline of Below Average SST Temperature Days
...
#| fig-height: 5
#| eval: true
#| fig-cap: "The total number of days below mean climatological average SST (blue) and days where SSTs were below the 10th percentile (light blue) from the climatological reference period (1991-2020)."
