...
```{r}
#| eval: false
#| label: monthly shift change

# Take the data that goes into each climatology,
# Plot the data points and boundaries as horizontal/vertical dashes colored by the climatology
# x = Month
# y = sst
# color = reference period
# 

clim_in_old <- sst_old %>% 
  mutate(yr = year(time), 
         mnth = month(time, label = T),
         mnth_num = as.numeric(mnth),
         ref_period = "1982-2011 Temperature") %>% 
  filter(yr %in% c(1982:2011))
clim_in_new <- sst_new %>% 
  mutate(yr = year(time), 
         mnth = month(time, label = T),
         mnth_num = as.numeric(mnth),
         ref_period = "1991-2020 Temperature") %>% 
  filter(yr %in% c(1991:2020))


# Combine
p_df <- bind_rows(clim_in_old, clim_in_new)

# Make monthly 10th/90th percentile averages
mnth_thresh <- p_df %>% 
  group_by(ref_period, mnth, mnth_num) %>% 
  summarise("Average" = mean(sst),
            "90th Percentile" = mean(mhw_thresh),
            "10th Percentile" = mean(mcs_thresh),
            .groups = "drop") %>% 
  pivot_longer(cols = c(4:6), names_to = "label", values_to = "val") %>% 
  mutate(mnth_num = ifelse(ref_period == "1982-2011 Temperature", mnth_num + 0.125, mnth_num-0.125))

# Make monthly Mean quartiles



# Put that big mess together
ggplot(p_df, aes(sst, mnth_num)) + 
  geom_point(
    data = clim_in_old,
    aes(sst, mnth_num+0.125 ,  color = ref_period), 
    shape = "|", alpha = 0.3) +
  geom_point(
    data = clim_in_new,
    aes(sst, mnth_num-0.125 ,  color = ref_period), 
  ...
  geom_segment(
    data = mnth_thresh,
    aes(x = val, xend = val, y = mnth_num+.15, yend = mnth_num-.15, color = label), linewidth = 1) +
  scale_color_manual(
    values = c(
      "10th Percentile" = gmri_cols("blue"),
      "1982-2011 Temperature" = "skyblue1",
      "1991-2020 Temperature" = "skyblue3",
      "90th Percentile" = gmri_cols("orange"),
      "Average" = "black")) +
  scale_y_reverse(
    breaks = c(12:1),
