...
For this seasonal report we present an analysis of SST for `r params$seas` `r month_range`. During this season, the average SST for the Gulf of Maine was `r season_avg_temp`°F, making it the `r rank_labs` hottest `r params$seas` on record for the period of 1982-2023 --- the period over which the satellite data used are available. This seasonal average temperature is `r season_avg_anom`°F above the 1991--2020 climatological reference period (CRP) `r params$seas` average of `r season_avg_clim`°F.
...
`r params$seas` average SST was on average `r round(mean(this_season_data$anom_f),2)`°F or more above normal during the fall, with weekly averaged SST anomalies ranging from -1.79°F to +3.85°F.
...
@tbl-monthly-temps shows monthly average SST for `r params$seas` `r params$year`; each month was within `r ceiling(min(monthly_avgs$anom_f))`°F or more above the 1991-2020 CRP. `r peak_month_full` showed the largest deviation from the long-term climatological average, with an average SST anomaly of `r peak_month$anom_f` °F, making it the 12th warmest October on record. September and November were cooler at 0.6 & 0.5°F above the long-term average—coming in as the 14th and 16th warmest September and November on record, respectively.
...
`r params$year` is the **`r rank_labs`** warmest `r params$seas` season observed in the Gulf of Maine during the 41 years we have satellite data to analyze, a departure `r params$seas` seasons in recent years, which experienced well-above average SSTs. 
...
The Gulf of Maine is an area of particular interest to the scientific community because of the remarkable rate of warming it has experienced in recent years coupled with its importance as a major driver for the regional economy.
 ...
 @fig-global-rates illustrates the average fall temperature anomaly for each year in the Gulf of Maine since the beginning of the satellite record in 1982 overlaid with long-term SST anomaly trends for the Gulf of Maine (orange) and global oceans (blue) overlaid. The `r params$seas` rate of warming for the Gulf of Maine (`r decadal_rate_f`°F per decade) is roughly 4x as fast as the warming rate for oceans globally (`r global_decadal_rate_f`°F per decade). The `r params$seas` SST anomaly for the Gulf of Maine in `r params$year` was 2.75°F cooler than that from 2022, the largest year-over-year decline in `r params$seas` temperatures in the satellite record. Despite the sizable year-over-year change, `r params$seas` `r params$year` remained warmer than 28 of the 29 fall seasons prior to 2010, consistent with the observed long-term warming trend in the region. 
...
region_hw %>% 
  ...
  geom_bracket(
    data = data.frame(xmin = 1991, xmax = 2020,y.position = .15, label = "1991-2020 Climatological Reference Period"), 
  ...
  labs(
    x = "Year", 
    y = str_c("Fraction of ",params$season, " Days"), 
    title = str_c("High Percentile ", params$season, " Warmth More Prevalent Since 2010"),
    fill = "SST Extremes")
...
Using this broadly accepted definition, **the Gulf of Maine has experienced MHW conditions for `r round(hw_frac)`% of this `r params$seas`**. September was the only month this fall where SST was above the threshold of a MHW, ultimately exhibiting these anomalously warm conditions for a total of six days. SSTs for the remainder of the season remained closer to the long-term average, even dipping below that climatological average for almost three weeks from late September into early October.
...
#| fig.height: 4
#| fig.align: center
#| fig-cap: "This series of maps shows the average monthly SST anomaly for September 2023, Ovtober 2023, and November 2023. The box outlined by the black dashed line denotes the region of study for the analysis. Darker red regions indicate warmer anomalies."
