...
month_summ_table <- monthly_avgs %>% 
  ...
  #rename(`Month Ranking` = rank) %>% 
  mutate(month = factor(month, levels = c("Dec", month.abb[1:11]))) %>% 
  gt(rowname_col = 'month') %>% 
  tab_header(
    title = md(paste0("**Monthly Sea Surface Temperatures - ", params$season, "**")), 
    subtitle = paste(month_range)) %>%
  ...
  # gt_plt_sparkline(
  #   temp_list, same_limit = T,fig_dim = c(8,30), 
  #   palette = c(
  #     "gray50",  # sparkline
  #     "black",  # final value
  #     gmri_cols("gmri blue"), # range color low
  #     gmri_cols("orange"), # range color high
  #     "black")) %>%
  cols_label(
      rank = "(1982-2023)",
      temp_f = "\u00b0F",
...
Using this broadly accepted definition, **the Gulf of Maine has experienced MHW conditions for `r round(hw_frac)`% of this `r tolower(params$season)`**. May was the only month this Spring where where SST was below the threshold of a MHW (90th Percentile). 
