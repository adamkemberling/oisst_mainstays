...
global_rate_plot <- ggplot(glob_plot_dat) +
  ...
  geom_mark_ellipse(aes(filter = region == "Global Oceans" & yr == 2023,
                        x = yr, 
                        y = anom_f,
                        label = region), 
  ...
  # Annotation for Warming Rate
  geom_mark_ellipse(
    aes(filter = region == "Gulf of Maine" & yr == 1999,
        x = yr, 
        y = anom_f,
        label = region), 
...
hw_temp_p <- year_hw_temps_two(
  ...
  temp_units = "F") + 
  labs(title = str_c(params$season, params$year, "Gulf of Maine SST", sep = " "),
