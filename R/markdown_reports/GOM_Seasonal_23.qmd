...
For this seasonal report we present an analysis of sea surface temperature (SST) for `r params$seas` 2023-2024, defined as the period extending from  `r month_range`. 
...
@tbl-monthly-temps shows monthly average SST for `r params$seas` `r params$year`. December SSTs were slightly below the 1991-2020 CRP, but the average SST anomalies for January and February were near or more than 1°F  below the 1991-2020 CRP.

February showed the largest deviation from the long-term climatological average, with an average SST anomaly of -1.08 °F, making it the 34th warmest February on record. Said another way, it was the 10th coldest February in the satellite era. January averaged –0.94 °F below the 1991-2020 climatology, making it the 25th warmest (19th coldest) January since 1982.

December was more mild, recording a monthly average SST that was 0.13 °F below the long-term average, respectively. This made December the 19th warmest (24th coldest) since 1982.
...
`r params$year` was the **`r rank_labs`** warmest (18th coolest) `r params$seas` season observed in the Gulf of Maine during the 41 years we have satellite data to analyze, a departure from `r params$seas` seasons in recent years, which experienced well-above average SSTs.
...
temp_rankings_p <- rank_plot_data %>% 
  ...
  #-------
  # # Year Labels - within bars
  # geom_text(
  #   mapping = aes(
  #     x = ifelse(anom_f < 0, anom_f, 0), 
  #     y = rank, 
  #     label = year_rank,
  #     color = I(ifelse(between(anom_f, -0.35, 0.35), "transparent", "white"))), # No numbering 
  #   fontface = "bold",
  #   family = "Avenir",
  #   hjust = -.05,
  #   #color = 'white',
...
The Gulf of Maine is an area of particular interest to the scientific community because of the remarkable rate of warming it has experienced in recent years, coupled with its importance as a major driver for the regional economy.
 ...
 @fig-global-rates illustrates the average fall temperature anomaly for each year in the Gulf of Maine since the beginning of the satellite record in 1982 overlaid with long-term SST anomaly trends for the Gulf of Maine (orange) and global oceans (blue) overlaid. 
 
 The `r params$seas` rate of warming for the Gulf of Maine (`r decadal_rate_f`°F per decade) is roughly ~3.3x as fast as the warming rate for oceans globally (`r global_decadal_rate_f`°F per decade). 
 
The `r params$seas` SST anomaly for the Gulf of Maine in `r params$year` was 4.29°F cooler than that from 2022, the largest year-over-year decline in `r params$seas` temperatures in the satellite record. 
...
The most commonly used definition of a ["marine heatwave" (MHW)](http://www.marineheatwaves.org/all-about-mhws.html) is when daily average SSTs exceeded the 90th percentile of a climatological (i.e., 30-year) average for at least 5 consecutive days. Gaps of 2 days or less in this threshold do not constitute a break in the MHW event. Using this broadly accepted definition, **the Gulf of Maine has experienced MHW conditions for `r round(hw_frac)`% of days this `r params$seas`**. 
...
hw_temp_p <- ggplot(year_hw_dat) +
    geom_path(aes(x = time, y = {{temp_col}}, 
                  color = mhw_event_txt, group = 1), 
...
clim_labels <- year_hw_dat %>% 
  ...
  pivot_longer(cols = -time, names_to = "clim_thresh", values_to = "temp_vals") %>% 
...
# Plot
hw_temp_p <- hw_temp_p +
    geom_line(aes(x = time, y = {{hw_thresh_col}}), color = "gray10", lty = 1) + 
    geom_line(aes(x = time, y = {{clim_col}}), color = "gray10", lty = 1) +
    geom_line(aes(x = time, y = {{cs_thresh_col}}), color = "gray10", lty = 1) +
...
#       hjust = 0.5, lty = 1, vjust = 0, family = "Avenir") + 
#     geom_textpath(
...
#       hjust = 0.5, lty = 1, vjust = .9, straight = T, family = "Avenir") +
#     # This only works if there are heatwaves
#     # geom_bracket(
#     #   data = hw_events, 
#     #   aes(xmin = llim, xmax = rlim, y.position = ymax + 2, label = str_c(len, " Days")), 
...
# # ------- Not Winter
# 
# # Weird behavior for winter - other months plot
# if(params$season != "Winter"){
#   hw_temp_p <- hw_temp_p +
#     geom_textpath(
#       aes(x = time, y = {{hw_thresh_col}}), 
#       color = "gray10", label = "90th Percentile", 
#       hjust = 1.35, lty = 1, vjust = 0, family = "Avenir") + 
#     geom_textpath(
#       aes(x = time, y = {{clim_col}}), 
#       color = "gray10", label = "Climatological Avg.", 
...
Presenting SST conditions in terms of anomalies ( @fig-season-anoms ) as opposed to absolute values ( @fig-season-mhw-timeline ) illustrates in greater detail the magnitude of MHW conditions throughout the `r params$seas`. 
...
#| fig-height: 5
#| eval: true
#| fig-cap: "A timeseries of daily average SST anomalies in the Gulf of Maine (solid colored line) compared to the long-term climatological mean, 10th, and 90th percentiles (solid black lines) in the Gulf of Maine for the period December 1, 2023 through February 28, 2024."
...
hw_anom_p <- year_hw_dat %>% 
  ...
  geom_ribbon(aes(x = time, ymin = cs_anom_thresh, ymax = hw_anom_thresh), fill = "lightgray", alpha = 0.4) +
    geom_path(aes(x = time, y = {{anom_col}}, color = mhw_event_txt, group = 1), linewidth = 1.2, key_glyph = "timeseries") +
    geom_textpath(
      aes(x = time, y = 0), 
      color = "gray10", 
      label = "Climatological Avg. & \n 10th-90th Percentile Range",
      vjust = 1.5,
      hjust = .9, lty = 1, family = "Avenir") +
  # Only Works if there are heatwaves
    geom_bracket(
      data = hw_events,
...
`r params$season` SSTs in the previous 3 years had been noteworthy for their extended MHW conditions, part of the long-term trend of unseasonably warm fall since 2012 (Figure 2). The relatively cold `r params$seas` in the Gulf of Maine this year were an extension of cooler fall conditions, but stand out against above-average winter conditions since 2012.
...
# All years:
heatmap_p <- grid_dat %>% 
  heatwave_heatmap_plot(temp_units = "F", start_yr = 1981, end_yr = params$year) +
  labs(
...
Average monthly SST anomalies are shown in @fig-monthly-avg-map. 
...
monthly_maps <- (
  # month_plots[[1]] + labs(title = "December 2024") | # WINTER control
  month_plots[[1]] |
    month_plots[[2]] | 
    month_plots[[3]]
    ) + 
