...
month_range <- switch(params$season,
  "Winter" = str_c("(December 1st, ", params$year-1," - February 28th, ", params$year,")"),
  "Spring" = str_c("(March 1st, ", params$year," - May 31st, ", params$year,")"),
  "Summer" = str_c("(June 1st, ", params$year," - August 31st, ", params$year,")"),
  "Fall" =   str_c("(September 1st, ", params$year," - November 30th, ", params$year,")"))
```
...
The observed SST, long-term average SST trends, and SST anomalies (i.e., departures from the long-term average SST) are shown. Departures from the long-term average were `r floor(mean(this_season_data$anom_f))`°F or more above normal during March & April, with temperatures in May falling closest to the long-term average.

```{r}
#| label: tbl-season-temps
#| tbl-cap: "Observed, climatological average, and deviation from the climatological average (i.e., temperature anomaly) for SST at a weekly resolution in the Gulf of Maine during spring 2023."
...
week_summ_table <- week_summaries  %>% 
  select(Week, temp_f, temp_c, clim_f, clim_c, anom_f, anom_c) %>%
   gt(rowname_col = "Week") %>% 
    tab_header(
    title = md(paste0("**Weekly Averaged Sea Surface Temperatures - ", params$season, "**")), 
    subtitle = paste(month_range)) %>%
    tab_stubhead(label = md("*One-Week Period*")) %>% 
    tab_spanner(
      label = md("*Observed Temperature*"), 
      columns = c(`temp_f`, `temp_c`)) %>% 
    tab_spanner(
      label = md("*Climatological Average*"), 
      columns = c(`clim_f`, `clim_c`)) %>% 
    tab_spanner(
      label = md("*Temperature Anomaly*"), 
      columns = c(`anom_f`, `anom_c`)) %>% 
    fmt_number(
      columns = c(temp_c, temp_f, clim_f, clim_c, anom_f, anom_c), 
      decimals = 2) %>% 
  # Change the column names to the units
   ...
   opt_table_font(
    font = list(google_font(name = "Raleway"))
    )
...
```{r}
#| label: monthly-temps
#| tbl-cap: "Observed, climatological average, and deviation from the climatological average (i.e., temperature anomaly) for SST at a monthly resolution in the Gulf of Maine during spring 2023 (defined as March 1st - May 30th)"
...
month_summ_table <- monthly_avgs %>% 
  ...
  mutate(month = factor(month, levels = c("Dec", month.abb[1:11]))) %>% 
  gt(rowname_col = 'month') %>% 
  tab_header(
    title = md(paste0("**Monthly-Averaged Sea Surface Temperatures - ", params$season, "**")), 
    subtitle = paste(month_range)) %>%
  tab_stubhead(label = md(""))  %>% 
  tab_spanner(label = md("*All-Year Rank*"), columns = c(rank)) %>% 
...
global_rate_plot <- ggplot(glob_plot_dat) +
  ...
  # geom_mark_ellipse(
  #   aes(filter = region == "Global Oceans" & yr == 2023,
  #       x = yr, 
  #       y = anom_f,
  #       label = "Global Oceans Trend"), 
  ...
  # # Annotation for Warming Rate
  # geom_mark_ellipse(
  #   aes(filter = region == "Gulf of Maine" & yr == 2014,
  #       x = yr, 
  #       label = "Gulf of Maine Trend",
  #       y = anom_f), 
...
#| fig.height: 6
#| fig-cap: "A ranking of the 5 warmest spring seasons for the Gulf of Maine in the satellite record (1982-2023). 2023 was the second warmest spring on record; the past three springs were each the warmest until that point; and the five warmest springs have all occurred since 2015."
...
temp_rankings_p <- seasonal_ranks %>% 
  ...
  geom_text(
    mapping = aes(
      x = 0, 
      y = year_rank, 
      #label = str_c(rank, ".  ", year_rank, " - ", as.numeric(as.character(year_rank))+1)), # WINTER Control
  ...
  scale_fill_manual(
    values = c(
      "flag" = as.character(gmri_cols("orange")), 
  ...
  coord_cartesian(clip = "off") +
  geom_logo(x_npc = 0.1, y_npc = -0.08, logo_height = 1, height_units = "cm")
...
Using this broadly accepted definition, **the Gulf of Maine has experienced MHW conditions for `r round(hw_frac)`% of this `r tolower(params$season)`**. May was the only month this Spring where SST was below the threshold of a MHW (90th Percentile). 
...
hw_temp_p <- year_hw_temps_two(
  year_hw_dat = last_yr_heatwaves, 
  temp_units = "F") + 
  labs(
    title = str_c(params$season, params$year, "Gulf of Maine SST", sep = " "))  +
  coord_cartesian(clip = "off") +
  geom_logo(x_npc = 0.9, y_npc = 1.12, logo_height = 1, height_units = "cm")
hw_temp_p
