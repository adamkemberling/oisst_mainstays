...
## New in 2023: Moving the Climatological Reference

This year's reporting will now be done using a new 30-year climatological reference period, a transition from to a 1991-2020 climate baseline. This change is in accordance with the standards set by international science community, following the [U.S. "Climate Normals"](https://www.ncei.noaa.gov/products/land-based-station/us-climate-normals) used by organizations like the [National Oceanic and Atmospheric Association](https://www.noaa.gov/) and the World Meteorological Organization.


```{r}
#| label: moving-climatology


# Take data from the four seasons and plot the distribution of temperatures over the different climatologies

szn_lvls <- c("Winter", "Spring", "Summer", "Fall")
hw_2011 <- region_timeseries %>% 
  filter(yr %in% c(1982:2011)) %>% 
  mutate(ref_period = "1982-2011", season_eng = factor(season_eng, levels = szn_lvls))
hw_2020 <- region_timeseries %>% 
  filter(yr %in% c(1991:2020)) %>% 
  mutate(ref_period = "1991-2020", season_eng = factor(season_eng, levels = szn_lvls))
ref_difference <- bind_rows(hw_2011, hw_2020)

median_df <- ref_difference %>% group_by(ref_period, season_eng) %>% summarise(med_sst = median(area_wtd_f, na.rm = T), .groups = "drop")


# Plot PDF
ggplot() +
  geom_density(data = filter(hw_2011, season_eng == params$season), 
               aes(x = area_wtd_f, fill = ref_period, color = ref_period), alpha = 0.3) +
  geom_density(data = filter(hw_2020, season_eng == params$season) , 
               aes(x = area_wtd_f, fill = ref_period, color = ref_period), alpha = 0.3) +
  geom_vline(data = filter(median_df, season_eng == params$season) , 
             aes(xintercept = med_sst, color = ref_period), linewidth = 1, show.legend = F) +
  # geom_mark_ellipse(data = filter(median_df, season_eng == params$season) , 
  #            aes(x = med_sst, y = 0.05, color = ref_period,
  #                label = str_c("Median Temperature: ", med_sst,deg_f)), show.legend = F, expand = .01, radius = 0.02, con.cap = 1) +
  scale_x_continuous(labels = number_format(suffix = deg_f)) + 
  scale_y_continuous(labels = label_percent()) + 
  scale_fill_gmri() +
  scale_color_gmri() +
  facet_wrap(~season_eng, ncol = 2, scales = "free") +
  theme(legend.position = "bottom") +
  labs(
    x = "Sea Surface Temperature", 
    y = NULL, 
    fill = "Climatological Reference", 
    color = "Climatological Reference") +
  guides(
...
For this seasonal report we present an analysis of SST for `r tolower(params$season)` `r month_range` for the year `r params$year`. This season the average SST for the Gulf of Maine was `r season_avg_temp`°F, making it the `r rank_labs` hottest `r tolower(params$season)` on record for the period of 1982-2023 --- the period over which the satellite data used are available. This seasonal average temperature is `r season_avg_anom`°F above the long-term (1991-2020) `r tolower(params$season)` average of `r season_avg_clim`°F.

## Weekly Temperatures

In the below table (@tbl-season-temps), we highlight how the SST for each week this `r tolower(params$season)` compares to a 30-year baseline period (i.e., climatological averages from 1991 through 2020).

The observed SST, long-term average SST, and SST anomalies (i.e., departures from the long-term average SST) are shown. SSTs in the `r tolower(params$season)` typically begin to slowly level out, before reaching their lowest around March. This pattern is evident this `r params$season` as expected, , with anomalies cresting over three periods within the course of the season. Departures from the long-term average hovered around `r floor(mean(this_season_data$anom_f))`°F above normal with a few weeks punctuated by anomalies in excess of 5°F, once in December, then again at the end of January, and again in mid-February.
...
week_summ_table <- week_summaries  %>% 
  select(Week, temp_f, temp_c, clim_f, clim_c, anom_f, anom_c) %>%
   gt(rowname_col = "Week") %>% 
    ...
    tab_source_note(md("*Climatology Reference Period: 1991-2020.*"))
...
Looking at monthly averages (as opposed to week-by-week conditions @tbl-monthly-temps), we see each month was `r min(monthly_avgs$anom_f)`°F or more above the 1991-2020 climatological average. `r peak_month_full` showed the largest deviation from the long-term climatological average, with an average SST anomaly of `r peak_month$anom_f`°F.
...
month_summ_table <- monthly_avgs %>% 
  ...
  cols_label(
   ...
   tab_source_note(source_note = md("*Data Source: NOAA OISSTv2 Daily Sea Surface Temperature Data.*") ) %>% 
   tab_source_note(source_note = md("*Climatology Reference Period: 1991-2020.*"))
...
```{r}
#| label: monthly-benchmarking
#| eval: false

this_yr_mnth_summ <- region_hw %>% 
  #filter(season_yr == params$year) %>% 
  filter(season_yr == 2022) %>% 
  group_by(season_yr, yr = year(time), month) %>% 
  summarise(
    # Uses Area Weighted SST
    mean_temp = mean(sst),
    mean_clim = mean(seas),
...
When looking at average `r tolower(params$season)` temperatures from the beginning of the satellite record in 1982, there is a clear long-term increase, with SST's warming at a rate of `r decadal_rate_f`°F per decade. This rate of warming is \~4x the rate that global ocean temperatures are warming during these same months, a rate of `r global_decadal_rate_f`°F per decade.

```{r}
#| label: global-rates-fig
#| eval: true
#| fig-cap: "Average annual winter SSTs in the Gulf of Maine from 1982 through 2022. The solid orange line indicates the trend for the full time series for the Gulf of Maine. A solid blue line indicates the trend for the full time series for the global oceans."
...
global_rate_plot <- ggplot(glob_plot_dat) +
  ...
  scale_y_continuous(
    ...
    # The rates are true but I think the globa temps are wrong?
    sec.axis = sec_axis(~ . + season_avg_clim,
                       labels = number_format(suffix = " \u00b0F"),
                       name = "Average SST")
    ) +
...
The unprecedented levels of warmth over the past decade or so are consistent with what researchers believe has been a distinct regime shift in terms of the major influences on SSTs in the Gulf of Maine. The drivers of this (e.g., a potential weakening of the Atlantic Meridional Overturning Circulation and widening of the Gulf Stream) have been [well-documented in the peer-reviewed literature](https://eos.org/features/why-is-the-gulf-of-maine-warming-faster-than-99-of-the-ocean), including through research by GMRI scientists.

## How Does `r params$year` Compare?

`r params$year` is the **`r rank_labs`** warmest `r tolower(params$season)` season observed in the Gulf of Maine during the 41 years we have satellite data to analyze. The top 5 warmest `r tolower(params$season)` seasons have all occurred in the last decade.

```{r}
#| label: fig-season-rank
#| fig.height: 4
#| fig-cap: "An ranking of the 5 warmest winter seasons for the Gulf of Maine in the satellite record (1982-2023). 2022 is the 2nd warmest winter on record; the five warmest winter seasons have all occurred within the last decade."
...
temp_rankings_p <- seasonal_ranks %>% 
  ...
  # Year Labels
  geom_text(
    mapping = aes(x = 0, y = year_rank, label = str_c(rank, ".  ", year_rank)),
    hjust = -.1,
    color = 'white',
    size = 5) +
  # Temperature Labels
...
The most commonly used definition of a ["marine heatwave" (MHW)](http://www.marineheatwaves.org/all-about-mhws.html) is when daily average SST's exceed the 90th percentile of a climatological (i.e., 30-year) average for at least 5 consecutive days. Gaps of 2 days or less in this threshold do not constitute a break in the MHW event.
...
Using this broadly accepted definition, **the Gulf of Maine has experienced MHW conditions for `r hw_frac`% of `r tolower(params$season)` `r params$year`**. A 16 day period over the New-Year's transition (Dec 27th - January 11th) were the only dates where SST was below the threshold for a MHW.

```{r}
#| label: fig-season-mhw-timeline
#| eval: true
#| fig-height: 4
#| fig-cap: "A timeseries of marine heatwave (MHW) conditions in the Gulf of Maine extending from December 1 2022 through February 28th, 2023. Black lines representing the long-term (i.e., 1991 – 2020) average SST, the 10th percentile, and 90th percentile for a given day in the Gulf of Maine are labelled to indicate climatological reference points; a solid line (red for marine heatwave or blue for a non-event) indicate the observed SST this year; red and blue shading illustrates how far the observed SST falls from the climatological mean."
...
#| fig-cap: "Heat map of daily SST anomalies from the beginning of 1982 through February 2023. Not only do more large warm anomalies (darker reds) appear more frequently in recent years, but the frequency and duration of marine heatwave events (black lines) in the Gulf of Maine has become more pronounced in the past decade."
...
heatmap_current <- grid_dat %>% 
  heatwave_heatmap_plot(temp_units = "F", start_yr = 2022, end_yr = 2023) +
  scale_y_continuous(labels = c("2022", "2023"), 
                     breaks = c(2022, 2023), 
                     limits = c(2021.5, 2023.5),
                     expand = expansion(add = c(0,0))) +
  theme(legend.position = "none",
        legend.title = element_text(angle = 0),
        plot.margin = margin(t = 0, b = 0),
        axis.title.x = element_blank()) +
  labs(title = "2022-2023 Conditions")
...
From a spatial perspective, the Gulf of Maine and surrounding areas experienced above average SST's for much of the region during `r tolower(params$season)` `r params$year`, but the warmest patches were to the south and east of Georges Bank, mostly outside the domain analyzed in preceding sections. The highest seasonally averaged anomaly of any location above was `r max_season`°F.
...
#| fig.width: 8
#| fig.height: 8
#| fig-cap: "Map of average SST anomalies for each grid cell in the satellite record for winter 2022-2023. The box outlined by the black dashed line denotes the region of study for the analysis (see @fig-map). Darker red regions indicate warmer anomalies."

# Make it a stars class to plot with ggplot
anoms_st <- st_as_stars(season_anoms_f)
...
Average monthly SST anomalies are shown in @fig-monthly-avg-map. The warmest anomalies were in December and concentrated to two pockets just south and to the south east of the domain studied in this analysis. During January a band of cold-water anomalies emerged to the southeast of these patches, but by February the areas just south and east of Georges Bank had spiked again. Monthly-averaged SST anomalies in the Gulf of Maine and the surrounding areas reached \> 8°F warmer than during our climate reference period (1991-2020).
