...
max_anom_yr <- seasonal_avgs %>% 
  ...
  slice(1) %>% 
  mutate(across(c(mean_temp:anom_f), \(x) sprintf(signif(x, 3), fmt = '%#.1f')))

```

For this seasonal report we present an analysis of SST for `r params$seas` `r month_range`. 
...
This year, the average SST for the Gulf of Maine was `r season_avg_temp`°F, making it the `r rank_labs` hottest `r params$seas` on record for the period of 1982-2023 --- the period over which the satellite data used are available. 

This seasonal average temperature is `r season_avg_anom`°F above the 1991--2020 climatological reference period (CRP) `r params$seas` average of `r season_avg_clim`°F.
...
`r params$seas` average SST was on average `r round(mean(this_season_data$anom_f),2)`°F or more above normal during the fall, with weekly averaged SST anomalies ranging from -1.7°F to +0.4°F.
...
week_summaries <- week_summaries %>%
  ...
  mutate(
    across(.cols = all_of(c("temp_c", "clim_c", "anom_c","temp_f", "clim_f", "anom_f")), 
           .fns = \(x) sprintf(signif(x, digits = 3), fmt = '%#.1f')))
...
@tbl-monthly-temps shows monthly average SST for `r params$seas` `r params$year`; each month was within `r ceiling(min(monthly_avgs$anom_f))`°F or more above the 1991-2020 CRP. 

`r peak_month_full` showed the largest deviation from the long-term climatological average, with an average SST anomaly of `r peak_month$anom_f` °F, making it the 34th warmest `r peak_month_full`` on record. 
...
`r params$year` was the **`r rank_labs`** warmest `r params$seas` season observed in the Gulf of Maine during the 41 years we have satellite data to analyze, a departure `r params$seas` seasons in recent years, which experienced well-above average SSTs. 
...
#     aes(yr, anom_f, shape = "Gulf of Maine Seasonal Average SST Anomaly"), 
#     color = "gray20", size = 2, alpha = 0.8) +
#   guides(shape = guide_legend(title = NULL,label.position = "left",label.hjust = 1))
...
global_rate_plot <- ggplot(glob_plot_dat) +
  ...
  geom_point(
    ...
    alpha = 0.8) +
  # # Add second legend
  # annotation_custom(grob = leg2, xmin = 2009, xmax = 2020, ymin = -3.6, ymax = -3.3) +
