...
For this seasonal report we present an analysis of SST for `r params$season` `r month_range` for the year `r params$year`. This season the average SST for the Gulf of Maine was `r season_avg_temp`°F, making it the `r rank_labs` hottest `r tolower(params$season)` on record for the period of 1982-2022 — the period over which the satellite data used are available. These temperatures are `r season_avg_anom`°F above the long-term (1982-2011) `r params$season` average of `r round(climate_baseline, 2)`°F.


## Weekly Temperatures

In the below table (@tbl-season-temps), we highlight how the SST for each week this `r tolower(params$season)` compares to a 30-year baseline period (climatological averages from 1982 through 2011).
...
#| tbl-cap: "Observed, climatological average, and deviation from the climatological average (i.e., temperature anomaly) for SST at a weekly resolution in the Gulf of Maine during summer 2022 (defined as September 1st - November 30th)"
...
# Make plots for each
month_plots <- imap(months_masked, monthly_sst_map, 
                    plot_yr = params$year,
