...
The average SST for the Gulf of Maine during the `r tolower(params$season)` of `r params$year` was `r season_avg_temp`°F. Making it the `r rank_labs` hottest `r tolower(params$season)` on record for the period of 1982-2022. These temperatures are `r season_avg_anom`°F above the long-term (1982-222) `r params$season` average of `r round(climate_baseline, 2)`°F.
...
month_full <- month.name[which(month.abb == peak_month$month)]
```


Looking at monthly averages (as opposed to week-by-week conditions), we see that `r month_full` showed the largest deviation from the long-term climatological average, with an average SST anomaly of `r peak_month$anom_f`°F. All days this season met the criteria to be considered a marine heatwave, and each month was `r min(monthly_avgs$anom_f)`°F or more above the 1982-2011 climatological average. 
...
month_summ_table <- monthly_avgs %>% 
  ...
  gt_sparkline(temp_list, 
               range_colors = c(gmri_cols("gmri blue")[[1]], 
                                gmri_cols("orange")[[1]]), 
               line_color = "gray50") %>%
  cols_label(
      temp_f = "\u00b0F",
...
When looking at average `r tolower(params$season)` temperatures, there is a clear long-term increase, with SST's warming at a rate of `r decadal_rate_f`°F per decade. A major factor driving this trend is a shift in the thermal regime that we began to observe around 2010.
...
ref_height <- switch(params$season,
  ...
  "Fall"   = data.frame(x = 1982, xend = 2011, y = 58, yend = 58))
...
seasonal_temp_dist <- ggplot() +
  ...
  geom_segment(data = ref_height, aes(x = x, xend = xend, y = y, yend = yend), color = "black") +
  geom_segment(data = ref_height, aes(x = x, xend = x, y = y-.5,   yend = y+.5), color = "black") +
  geom_segment(data = ref_height, aes(x = xend, xend = xend, y = y-.5,   yend = y+.5), color = "black") +
  annotate(x = 1997, y = ref_height$y + 1.5, label = "Climate-Reference Period", geom = "text", size = 3) +
...
`r params$year` is among the warmest `r tolower(params$season)`s seen in the Gulf of Maine during this 40+ year period. When compared against all previous `r tolower(params$season)`s for which we have reliable satellite data (i.e., back to 1982), this year ranks **`r rank_labs`** among the hottest on record since 1982.
...
month_rank_heatmap(hw_dat = grid_dat, 
                   temp_units = "F",
                   no_dates_before = "2022-09-01") +
  coord_cartesian(clip = "off") +
...
# Plot it out:
event_temps <- year_hw_temps_two(year_hw_dat = longest_hw, temp_units = "F") +
  scale_x_date(
    date_labels = "%b %y", 
    date_breaks = "2 month", 
...
# Anomalies
event_anoms <- year_hw_anoms_two(year_hw_dat = longest_hw, temp_units = "F") +
  scale_x_date(
    date_labels = "%b %y", 
    date_breaks = "2 month", 
    expand = expansion(mult = c(0,0)), 
    guide = guide_axis(n.dodge = 1)) +
  theme(legend.position = "none") +
  labs(title = "Gulf of Maine's Record Heatwave", subtitle = "Temperature anomalies for the longest heatwave on record.") +
  coord_cartesian(clip = "off") +
  geom_logo(x_npc = 0.9, y_npc = 1.1, logo_height = 1, height_units = "cm")
...
This event lasted a total of `r hw_length`, and during that period temperatures were on average `r avg_hw_temp`°F above temperatures observed during the climate reference period.
