...
If we use the standard MHW definition, where heatwaves and coldspells are determined using a static 30-year baseline we get the following record of these extreme events:
...
If instead, we adopt an approach which removes the long-term warming trend we get the following figure.
...
There is debate on the merits of either approach, and further discussion on whether regional dynamics (e.g. the North Atlantic Oscillation, ENSO, Gulf Stream Position) should be accounted for when identifying marine heatwaves.
