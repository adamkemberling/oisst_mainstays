...
if(params$season == "Winter"){
  ...
  seasonal_ranks <- seasonal_ranks %>% mutate(
...
week_summ_table <- week_summaries %>% 
  ...
  select(Week, temp, clim, anom) %>%
   gt(rowname_col = "Week") %>% 
  tab_header(
    title = md(paste0("Table 1. Weekly Average Sea Surface Temperatures - ", params$season)), 
...
peak_month <- monthly_avgs %>% slice_max(order_by = abs(anom_f), n = 1)
...
month_summ_table <- monthly_avgs %>% 
  ...
  tab_header(
    title = md(paste0("Table 2. Monthly Average Sea Surface Temperatures - ", params$season, "")), 
    subtitle = month_range) %>%
...
`r params$year` was the **`r rank_labs`** warmest `r params$seas` season observed in the Gulf of Maine during the 42 years we have satellite data to analyze. Its position among years ranked more-highly is displayed below in @fig-season-rank.
...
rank_filter_depth <- ifelse(
  current_rank <= 10,
  20,
  DescTools::RoundTo(current_rank, multiple = 5, FUN = ceiling)
)
...
hw_temp_p <- hw_temp_p +
    ...
    # this is breaking for some reason with winter
    scale_x_date(
      date_labels = "%b",
      date_breaks = "1 month",
...
### Competing Philosophies on Marine Heatwaves
...
fixed_hw <- hw_dat %>% 
   mutate(
    ...
    theme(
      ...
      plot.margin = margin(b = 0, t = 20, l = 3, r = 3),
...
# Combine them
heatmap_p <- (fixed_hw | (detrend_hw + labs(title = ""))
