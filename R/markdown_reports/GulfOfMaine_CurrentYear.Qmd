...
ggplot(data = one_yr, aes(x = flat_date)) +
    ...
    geom_textline(aes(y = mhw_thresh, color = "90th Percentile", label = "Heatwave Threshold"), linetype = 1, size = 5, hjust = 0.25, linewidth = 1) +
    geom_textline(aes(y = mcs_thresh, color = "10th Percentile", label = "Coldspell Threshold"), linetype = 1, size = 5, hjust = 0.75, linewidth = 1) +
  ...
  scale_y_continuous(labels = number_format(suffix = " \u00b0C")) +
    ...
    theme(
      ...
      guides(color = guide_legend(
        ...
        label.position = "top", 
        override.aes = list(
          color = c( "royalblue","coral3",  "gray10", "gray50"),
          shape = c(NA, NA, NA, 16),  
          linetype = c(1, 1, 1, NA),
          alpha = c(1,1,1,1), 
          label = c("", "", "", "")),
...
last_yr_heatwaves <-  region_hw %>% 
  filter(year(time) >= last_year,
         time <= as.Date("2022-06-26"))
