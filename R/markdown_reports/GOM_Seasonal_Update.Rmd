...
For the data within this seasonal report, include data for `r params$season` following the meteorological definitions for the seasons. This is in contrast to the astrological seasons, which track with Earth's tilt in relationship to the sun. Using this meteorological system the first days of March, June, September, and December each mark the start of Meteorological Spring, Summer, Fall and Winter respectively. 
...
ggplot(met_seasons_df) +
  ...
  geom_text(aes(x = labeldate, y = label_y, label = season), 
            color = met_seasons_df$current_season) +
...
Knowing how hot or cold it was in the past gives us a baseline that we can compare with temperatures we observe today. In the below table, we highlight how the surface ocean temperature for each week this summer (observed SST) compares to a 30-year baseline period (climate average) that spans from 1982 to 2011. The table shows the absolute values as well as the departures from that long-term average (temperature anomalies).

We can learn two important lessons by looking at data this way:

 1. The SST anomalies in July were lower than they were in June or August, meaning the month of July was relatively cooler (it was also significantly wetter).   
 2. **Even in that “cool” month of July, the Gulf of Maine was still close to 2°C warmer than normal** (and almost 3°C warmer than normal for stretches in June and August).   
...
seasonal_temp_p <- ggplot(seasonal_avgs, aes(yr, mean_temp)) +
  ...
  geom_smooth(data = filter(seasonal_avgs, yr %in% c(2011:2021)),
              aes(yr, mean_temp), linetype = 1,
              formula = y~x, method = "lm", 
              color = gmri_cols("gmri blue"), 
              se = FALSE) +
  geom_mark_ellipse(aes(filter = yr == 2002, 
                        label = str_c(decadal_rate, " \u00b0C / Decade")), 
                    color = "transparent", 
                    description = "40-year warming trend", 
                    label.colour = "darkred",
                    con.colour = "darkred",
                    label.buffer = unit(1.25, "cm"), 
                    label.fill = "transparent", 
                    label.fontsize = 11) +
  geom_mark_ellipse(aes(filter = yr == 2016, 
                        label = str_c(tenyr_decadal, " \u00b0C / Decade")), 
                    color = "transparent", 
                    description = "Ten-year warming rate", 
  ...
  labs(x = "",
       ...
       subtitle = str_c("Average ", params$season, " Temperatures: Gulf of Maine"))
...
season_anom_p <- ggplot(region_timeseries, aes(time, area_wtd_anom)) +
  ...
  ggforce::geom_mark_ellipse(data = filter(seasonal_avgs, yr == warmest_year), 
                             aes(x = season_center, 
                                 y = mean_anom, 
