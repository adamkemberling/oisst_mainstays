...
The average SST for the Gulf of Maine during the `r tolower(params$season)` of `r params$year`  (`r season_range`) was `r season_avg_temp`°F, which is `r season_avg_anom`°F above the long-term average of `r round(as_fahrenheit(climate_baseline), 2)`°F.

These facts describe what make it the `r rank_labs` hottest `r tolower(params$season)` on record for the period of 1982-2022. Read on to learn more about what SST conditions were like in the Gulf of Maine during this `r tolower(params$season)`.


## Weekly Temperatures: `r params$season`


Knowing how hot or cold it was in the past gives us a baseline against which we can compare temperatures we observe today. In Table 1, we highlight how the surface ocean temperature for each week this `r tolower(params$season)` (observed SST) compares to a 30-year baseline period (climate average) that spans from 1982 to 2011. The table shows the absolute values as well as the departures from that long-term average (temperature anomaly). 
...
Over the period of 1982-2021, `r tolower(params$season)` temperatures in the Gulf of Maine have warmed at a rate of **`r decadal_rate`** °C per Decade.
...
The Gulf of Maine began this year experiencing MHW conditions spilling over from an event the previous year. That event began at the start of 2021, lasting for the entirety of the year, and persisting through the Spring of 2022. This is the longest heatwave on record for the Gulf of Maine, .
...
avg_hw_temp <- mean(longest_hw$anom_f)
