...
In the below table, we highlight how the surface ocean temperature for each week this `r params$season` compares to a 30-year baseline period (climatological averages from 1982 to 2011). 
...
monthly_avgs <- season_data %>% 
  ...
  summarise(
    ...
    anom_f = as_fahrenheit(anom_c, data_type = "anomalies"),
    #anom_list = list(sst_anom), # list column for sparkline anomalies
    temp_list = list(sst), # list column for sparkline anomalies
...
seasonal_temp_dist <- ggplot(
  ...
  labs(x = "",
       y = str_c(params$season, " Sea Surface Temperature"),
       title = str_c( params$season, " Temperatures Reflect Regime Shift"),
       color = "") +
