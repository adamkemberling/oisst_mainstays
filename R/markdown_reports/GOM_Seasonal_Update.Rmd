...
The spatial extent that we've used as the "Gulf of Maine" when processing satellite data for analyses or figures is displayed below. This area is consistent with reports done in previous years and in earlier publications.
...
## Weekly `r params$season` Temperatures

Area-specific time series are the most basic building block for relaying temporal trends. For the Gulf of Maine (and other areas around the world!) we can generate a time series of the mean sea surface temperature for each day for as long as we have satellite data.

By knowing the hot/cold variation that it was in the past we are able to compare how recent temperatures compare to conditions of the past. 

This section highlights how the surface ocean temperatures of recent weeks compare to the climate average across our reference period of 1982-2011.
...
# Get heatwave status for each day:
# Uses area wtd sst by default
...
#### BUG FIXING:
#lubridate::week()
# does not align weeks the same as floor_date and ceiling_date
# outcome is week ID's that don't match floor/ceiling bookends
...
Over the period of 1982-2020 the Gulf of Maine has warmed at a rate of **`r decadal_rate`** $\frac{^oC}{Decade}$, and more recently at a rate of **`r tenyr_decadal`** $\frac{^oC}{Decade}$ for the last decade (2011-2021). 
...
seasonal_temp_p <- ggplot(seasonal_avgs, aes(yr, mean_temp)) +
  geom_line(color = "gray20", linetype = 3, size = 0.5, alpha = 0.8) +
  geom_point(color = "gray20", size = 1, alpha = 0.8) +
  geom_smooth(formula = y~x, method = "lm", color = "darkred", se = FALSE) +
  geom_smooth(data = filter(seasonal_avgs, yr %in% c(2011:2021)),
              aes(yr, mean_temp), linetype = 4,
  ...
  labs(x = "",
       ...
       subtitle = str_c("Long-Term Increase in Average ", params$season, " Temperatures"))
...
While the rate of summer warming has slowed, the average temperature over the three-month period has not fallen below $15^oC$ since 2007. Prior to 2007 this had only occurred seven times in 25 years (1982-2007).

The average temperature for this `r tolower(params$season)` was `r seas_temp`$^oC$ / `r round(as_fahrenheit(seas_temp), 2)`$^oCF$. 
...
seasonal_temp_dist <- ggplot(
  filter(region_timeseries, season_eng == params$season), 
  ...
  annotate(x = 1997, y = 21.5, label = "Climate-Reference Period", geom = "text", size = 3) +
  geom_hline(linetype = 2, size = 0.75, aes(yintercept = climate_baseline, color = "30-Year Climate Average")) + 
  scale_y_continuous(sec.axis = sec_axis(trans = ~as_fahrenheit(., data_type = "anomalies"), name = expression(Temperature~degree*F))) +
  labs(x = "", y = expression(Temperature~degree*C),
       subtitle = str_c("Majority of ", params$season, " Temperatures Now Exceed Climate Average"),
       color = "") +
  scale_color_manual(values = c("30-Year Climate Average" = "darkred")) +
  theme(legend.position = c(0.78, 0.125), 
...
`r params$season`temperatures since 2008 have not fallen below $0.5~^oC$ with `r max_anom_yr$yr` remaining the hottest summer at `r max_anom_yr$mean_anom`$~^oC$ above the 30-year summer average.

```{r seasonal anom plots}
# # Figure
...
#        subtitle = str_c(params$season, " Sea Surface Temperature Anomalies")) + 
#   theme(
#     axis.title.y.left = element_text(angle = 0, hjust = 0.5, vjust = 0.5),
...
The average temperature anomaly for this `r tolower(params$season)` was `r seas_anom`$^oC$ / `r round(as_fahrenheit(seas_anom, "anomalies"), 2)`$^oCF$.



```{r daily anomalies figure, fig.height = 6, fig.width=8}

# Plot the daily anomaly pattern 
season_anom_p <- ggplot(region_timeseries, aes(time, area_wtd_anom)) +
  ...
  ggforce::geom_mark_ellipse(data = filter(seasonal_avgs, yr == 2012), 
                             aes(x = season_center, 
                                 y = mean_anom, 
                                 label = str_c(round(mean_temp, 2), "\u00b0", "C  |  ", round(temp_f, 2), "\u00b0", "F")), 
                             description = str_c("Warmest ", params$season, " on Record"), 
                             label.fill = "transparent", label.fontsize = 8, 
                             linetype = 2,
                             label.buffer = unit(0.5, "cm")) +
  scale_y_continuous(sec.axis = sec_axis(trans = ~as_fahrenheit(., data_type = "anomalies"), name = expression(Temperature~Anomaly~degree*F))) +
  labs(x = "", y = expression(Temperature~Anomaly~degree*C), subtitle = str_c(params$season, " Sea Surface Temperature Anomalies"))
...
# Plot them
ggplot(seasonal_ranks, aes(x = mean_anom, xend = 0, 
                          y = fct_rev(year_rank),
                          yend = fct_rev(year_rank), 
                          color = flag_year) ) +
  geom_segment(size = 1) +
  geom_point(aes(size  = flag_year)) +
  ...
  scale_size_manual(values = c("flag" = 3, "unflag" = 1)) +
  labs(x = expression(Temperature~degree*C),
       y = "",
       subtitle = "Seasonal Temperature Rankings:") +
...
We also see a less-pronounced pause than in summer temperature increases, an indication that other seasons are continuing to warm even if the Summer warming has slowed.
...
annual_trend_p <- ggplot(data = annual_summary, aes(yr_as_dtime, area_wtd_anom)) +
  ...
  geom_smooth(data = filter(annual_summary, year <= 2020),
              ...
              formula = y ~ x, se = F,
              linetype = 1) +
  # geom_smooth(data = filter(annual_summary, year %in% c(2011:2021)),
  #             method = "lm", 
  #             aes(color = "2011-2020 Regional Trend"),
  #             formula = y ~ x, se = F, 
  #             linetype = 2) +
  ...
  # geom_text(data = data.frame(), 
  #           aes(label = eq_10, x = min(region_timeseries$time), y = Inf),
  ...
  scale_color_manual(values = c(
    ...
    # "2011-2020 Regional Trend" = as.character(gmri_cols("orange")),
...
region_heatwaves <- region_hw %>% 
  mutate(year = year(time),
         yday = yday(time))


#### This section is mess b/c I am bouncing back and forth from last 365 days or the entire year
...
# Get number of heatwave events and total heatwave days for this
...
Over the course of 2021 the Gulf of Maine has been in heatwave conditions for much of the year. Heatwave conditions began in January that persisted through until march. Another brief event happened mid-march, before a third event which ended in the end of April. 

The longest heatwave of the year began at the very end of April, lasting until the end of July, peaking at nearly $4~^oC$ above what we would have considered "normal" for that time of year. Now back in August we again are in heatwave conditions in the $2-3~^oC$ range for temperature anomalies.
...
# ggforce is cool but does not work well with patchwork:
# ggforce::geom_mark_ellipse(data = data.frame("lon" = c(-71, -65, -65, -70), 
#                                              "lat" = c( 45.1, 45.1, 40.3, 40.3),
