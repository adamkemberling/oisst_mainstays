...
# Fahrenheit
seasonal_temp_f <- ggplot(seasonal_avgs, aes(yr, temp_f)) +
  geom_line(color = "gray20", linetype = 3) +
  geom_point(color = "gray20") +
  geom_smooth(formula = y~x, method = "lm", color = gmri_cols("orange"), se = FALSE) +
  geom_smooth(data = filter(seasonal_avgs, yr %in% c(2011:2021)),
              aes(yr, temp_f), linetype = 2,
              formula = y~x, method = "lm", color = gmri_cols("gmri blue"), se = FALSE) +
  labs(x = "Year",
       y = expression("SST"~~degree*"F"))
...
seasonal_anom_c <- ggplot(seasonal_avgs, aes(yr, mean_anom)) +
  ...
  geom_smooth(data = filter(seasonal_avgs, yr %in% c(2011:2021)),
              ...
              formula = y~x, method = "lm", color = gmri_cols("gmri blue"), se = FALSE) +
  labs(x = "",
       y = expression("SST Anomaly"~~degree*C),
       subtitle = str_c(params$season, " Sea Surface Temperature Anomaly"))

# Fahrenheit
seasonal_anom_f <- ggplot(seasonal_avgs, aes(yr, anom_f)) +
  geom_line(color = "gray20", linetype = 3) +
  geom_point(color = "gray20") +
  geom_smooth(formula = y~x, method = "lm", color = gmri_cols("orange"), se = FALSE) +
  geom_smooth(data = filter(seasonal_avgs, yr %in% c(2011:2021)),
              aes(yr, anom_f), linetype = 2,
              formula = y~x, method = "lm", color = gmri_cols("gmri blue"), se = FALSE) +
  labs(x = "Year",
       y = expression("SST Anomaly"~~degree*"F"))


# Show figure
seasonal_anom_p <- seasonal_anom_c / seasonal_anom_f
seasonal_anom_p
