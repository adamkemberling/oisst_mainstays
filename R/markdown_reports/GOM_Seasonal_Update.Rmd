...
climate_baseline <- this_seas$mean_clim
...
**Note on Preliminary Data:** This dataset has a 2-week latency period, during which values may change due to quality control measures. Any data within 14 days of the current date may be subject to change.
...
For this seasonal report, we include data for `r params$season` that follows the meteorological definitions for the seasons. Under this system the first days of March, June, September, and December each mark the start of Meteorological Spring, Summer, Fall and Winter respectively.
...
The average sea surface temperature (SST) for the Gulf of Maine during the `r tolower(params$season)` of `r params$year`  (`r season_range`) was `r season_avg_temp`°F, which is `r season_avg_anom`°F above the long-term average of `r round(as_fahrenheit(climate_baseline), 2)`°F.

These values make it the `r rank_labs` hottest `r tolower(params$season)` on record for the period of 1982-2022. Read on to learn more about what SST conditions were like in the Gulf of Maine during this `r tolower(params$season)`.


## Weekly Temperatures: `r params$season`

In the below table, we highlight how the SST for each week this `r tolower(params$season)` compares to a 30-year baseline period (climatological averages from 1982 to 2011).

The absolute sea surface temperatures, the long-term average, as well as departures from the long-term average (temperature anomalies) are shown.
...
**Looking at the data this way reveals a stunning result:** SSTs in the Gulf of Maine rose above 40°F in late February, 5-6 weeks sooner than they have historically.
...
decadal_rate_f <- round(as_fahrenheit((annual_rate*10), "anomalies" ), 2)
...
seasonal_temp_dist <- ggplot(
  ...
  geom_hline(linetype = 1, size = 0.75, 
             aes(yintercept = as_fahrenheit(climate_baseline), 
...
`r params$season` temperatures since 2008 have not fallen below 0.5°C with `r max_anom_yr$yr` positioned as the hottest `r tolower(params$season)` at `r max_anom_yr$mean_anom`°C above the 30-year summer average.

The average temperature anomaly for this `r tolower(params$season)` was `r seas_anom`°C / `r round(as_fahrenheit(seas_anom, "anomalies"), 2)`°F.
...
regional_anom_p <- ggplot(region_timeseries, aes(time, anom_f)) +
...
`r params$year` is among the warmest `r tolower(params$season)`s seen in the Gulf of Maine during this 40+ year period. When compared against all previous `r tolower(params$season)`s (1982-2021), this year ranks **\# `r current_rank`** for hottest on record.
...
A [marine heatwaves](http://www.marineheatwaves.org/all-about-mhws.html) (MHW) is defined as a situation when SSTs exceed a threshold (usually the 90th percentile of prior daily observations) for at least 5 consecutive days. Successive heatwaves with gaps of 2 days or less are considered part of the same event. The heatwave threshold used below was 90%.
...
Using this broadly accepted definition, **the Gulf of Maine has experienced MHW conditions for almost all of 2022**. There was a very brief two-day reprieve in mid-May which was followed by a sharp  climb in temperature soon-after. Because of that brevity scientists would consider the observed temperatures to be part of the same heatwave event.

The Gulf of Maine began this year experiencing MHW conditions spilling over from an event the previous year. That event began at the start of 2021 and lasted for the entirety of the year. The heatwave seen in the figure below is the tail end of that event, which persisted all the way through to June 2022.
...
Changing the axis to anomalies rather than temperature, the magnitude of the MHW become more clear. The most extreme anomalies of this `r params$season` occurred at the beginning of January and in the second-half of May, with temperatures exceeding $4$°F above the climatological average. Remember, the value of zero in this figure is the average daily temperature from the climate period of 1982-2011. Not a single day in 2022 has been $1.5$°F of that average temperature.
...
event_temps <- year_hw_temps(year_hw_dat = longest_hw, temp_units = "F") +
  ...
  labs(title = "Gulf of Maine's Record Heatwave",
       subtitle = str_c("Heatwave event lasting ", hw_length, " days sets new record as longest single heatwave event."))
...
# Convert to F
season_mean <- as_fahrenheit(season_mean, data_type = "anomalies")
...
if(params$`Save Figures`){
  ...
  # Gom Extent Map
  ggsave(plot = season_polar, 
         filename = str_c(update_folder, "met_seasons.png"), 
         dpi = "retina", 
         bg = "transparent")
