...
Average monthly temperatures are displayed below:

```{r month avgs, eval = TRUE, layout="l-body-outset", fig.width=6, fig.height=4}
# Load the monthly average array
monthly_avgs <- stack(str_c(oisst_path, "monthly_averages/oisst_monthly.nc"), varname = "sst")
...
# Clip to the study exxpanded area
# Expanded area
...
month_plots <- imap(months_masked, function(month_avg_layer, month_id){
  ...
  # # tester
  # month_avg_layer <- months_masked[[1]]
  # month_id <- names(display_months[[1]])
  
  # Set up text label  
  month_label <- str_sub(month_id, 2, -1)
  month_label <- str_replace(month_label, "[.]", "-")
  ...
  month_fig <- ggplot() +
    ...
    coord_sf(xlim = crop_x, 
             ...
             expand = T) +
    guides("fill" = guide_colorbar(
      title = expression(" Average Temperature"~degree*C),
      title.position = "top",
      title.hjust = 0.5,
      barwidth = unit(3, "in"),
      frame.colour = "black",
